...
void FDefaultXRCamera::PreRenderViewFamily_RenderThread(FRDGBuilder& GraphBuilder, FSceneViewFamily& ViewFamily)
{
	...
	if (TrackingSystem->DoesSupportLateUpdate())
	{
		...
		auto CVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.EnableLateLatching"));
		if (CVar && !CVar->GetValueOnAnyThread() && ViewFamily.bLateLatchingEnabled)
		{
			ViewFamily.bLateLatchingEnabled = false;
			UE_LOG(LogHMD, Warning, TEXT("LateLatching is disabled by r.EnableLateLatching."));
		}
	}
	// END META SECTION - PORT PICO LateLatching
	{
