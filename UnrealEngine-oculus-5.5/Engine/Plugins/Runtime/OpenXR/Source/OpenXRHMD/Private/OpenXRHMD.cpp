...
static TAutoConsoleVariable<bool> CVarOpenXRInvertAlpha(
	...
	false,
	// BEGIN META SECTION - OpenXR inverted alpha extension
	TEXT("Enables alpha inversion of the backgroud layer if the XR_EXT_composition_layer_inverted_alpha extension is supported.\n"),
	// END META SECTION - OpenXR inverted alpha extension
	ECVF_Default);
...
bool FOpenXRHMD::GetStereoFieldOfView(int32 ViewIndex, float& OutAngleLeft, float& OutAngleRight, float &OutAngleUp, float &OutAngleDown) const
{
	FPipelinedFrameStateAccessorReadOnly LockedPipelineState = GetPipelinedFrameStateForThread();
	const FPipelinedFrameState& FrameState = LockedPipelineState.GetFrameState();

	if (FrameState.Views.IsValidIndex(ViewIndex))
	{
		OutAngleLeft =  FrameState.Views[ViewIndex].fov.angleLeft;
		OutAngleRight = FrameState.Views[ViewIndex].fov.angleRight;
		OutAngleUp = FrameState.Views[ViewIndex].fov.angleUp;
		OutAngleDown = FrameState.Views[ViewIndex].fov.angleDown;
...
// BEGIN META SECTION - OpenXR MultiPlayer
#if WITH_EDITOR
	FTransform Pose(CurrentOrientation, CurrentPosition);
	for (IOpenXRExtensionPlugin* Module : ExtensionPlugins) 
	{
		Module->ReCalcPose(Pose);
	}
	CurrentOrientation = Pose.GetRotation();
	CurrentPosition = Pose.GetTranslation();
#endif // WITH_EDITOR
// END META SECTION - OpenXR MultiPlayer
			...
			if (GIsEditor)
			{
				if (FSceneViewport* SceneVP = FindSceneViewport())
				{
					...
					if (Window.IsValid())
					{
						...
						CalculateRenderTargetSize(SizeX, SizeY);
...
// BEGIN META SECTION - Stereo Render Target Size
void FOpenXRHMD::CalculateRenderTargetSize(uint32& InOutSizeX, uint32& InOutSizeY)
// END META SECTION - Stereo Render Target Size
{
	...
	FPipelinedFrameStateAccessorReadOnly LockedPipelineState = (const_cast<const FOpenXRHMD*>(this))->GetPipelinedFrameStateForThread();
	const FPipelinedFrameState& PipelineState = LockedPipelineState.GetFrameState();
// BEGIN META SECTION - Dynamic Resolution
	float PixelDensity = PipelineState.PixelDensity;

	for (IOpenXRExtensionPlugin* Module : ExtensionPlugins)
	{
		PixelDensity = FMath::Max(PixelDensity, Module->GetMaxPixelDensity());
	}
// END META SECTION - Dynamic Resolution
...
// BEGIN META SECTION - Dynamic Resolution
		const FIntPoint DensityAdjustedSize = GeneratePixelDensitySize(Config, PixelDensity);
// END META SECTION - Dynamic Resolution
...
bool FOpenXRHMD::AllocateSwapchainTextures_RenderThread(const FOpenXRSwapchainProperties& InSwapchainProperties, FXRSwapChainPtr& InOutSwapchain, uint8& OutActualFormat)
{
	...
	if (InOutSwapchain == nullptr ||
		...
		// BEGIN META SECTION - Add XR Cubemap Support
		SwapchainTexture->GetDesc().Dimension != InSwapchainProperties.Dimension ||
		// END META SECTION - Add XR Cubemap Support
		SwapchainTexture->GetFlags() != InSwapchainProperties.CreateFlags)
	{
		InOutSwapchain = RenderBridge->CreateSwapchain(
			...
			// BEGIN META SECTION - Add XR Cubemap Support
			InSwapchainProperties.Dimension,
			// END META SECTION - Add XR Cubemap Support
		...
		if (InOutSwapchain)
		{
			UE_LOG(LogHMD, Verbose, TEXT("Allocated %.*s OpenXR Swapchain of Format: %u, SizeX: %u, SizeY: %u, ArraySize: %u, NumMips: %u, NumSamples: %u, Dimension: %u, Flags: %llu"),
...
bool FOpenXRHMD::AllocateRenderTargetTextures(uint32 SizeX, uint32 SizeY, uint8 Format, uint32 NumMips, ETextureCreateFlags Flags, ETextureCreateFlags TargetableTextureFlags, TArray<FTextureRHIRef>& OutTargetableTextures, TArray<FTextureRHIRef>& OutShaderResourceTextures, uint32 NumSamples)
{
	...
	// BEGIN META SECTION - OpenXR LDR Tonemap color space fix
	// In case of LDR tonemap , the color is changed to sRGB within the shader, so we need to allocate unorm to avoid HW color conversion
	bool isTonemapEnabled = IsMobileTonemapSubpassEnabledInline(GetConfiguredShaderPlatform(), bIsMobileMultiViewEnabled, NumSamples);
	bool MobileHWsRGB = IsMobileColorsRGB() && IsMobilePlatform(GetConfiguredShaderPlatform()) && !isTonemapEnabled;
	// END META SECTION - OpenXR LDR Tonemap color space fix
	if (MobileHWsRGB)
	{
	...
	for (IOpenXRExtensionPlugin* Module : ExtensionPlugins)
	{
		Module->AllocateRenderTargetTextures_RenderThread();
	}
	// END META SECTION - OpenXR AppSpaceWarp
...
// BEGIN META SECTION - OpenXR AppSpaceWarp
bool FOpenXRHMD::GetRecommendedVelocityTextureSize(FIntPoint& OutTextureSize)
{
	check(IsInRenderingThread());

	for (IOpenXRExtensionPlugin* Module : ExtensionPlugins)
	{
		if (Module->GetRecommendedVelocityTextureSize_RenderThread(OutTextureSize))
		{
			return true;
		}
	}
...
bool FOpenXRHMD::AllocateVelocityTexture(uint32 Index, const FIntPoint& Size, uint8 Format, uint32 NumMips, ETextureCreateFlags Flags, FTextureRHIRef& OutTexture, uint32 NumSamples)
{
	check(IsInRenderingThread());

	FXRSwapChainPtr VelocitySwapchain = nullptr;
	for (IOpenXRExtensionPlugin* Module : ExtensionPlugins)
	{
		if (FXRSwapChainPtr Swapchain = Module->GetVelocitySwapchain_RenderThread())
		{
			if (Swapchain)
			{
				VelocitySwapchain = Swapchain;
			}
		}
	}
	...
	{
		const FRHITexture* const VelocitySwapchainTexture = VelocitySwapchain->GetTexture2DArray() ? VelocitySwapchain->GetTexture2DArray() : VelocitySwapchain->GetTexture2D();
		const FRHITextureDesc& VelocitySwapchainDesc = VelocitySwapchainTexture->GetDesc();
		
		if (Size != VelocitySwapchainDesc.Extent)
		{
			// We don't yet support velocity texture sizes other than the recommended size
			return false;
		}

		// Sample count, mip count and size should be known at AllocateRenderTargetTexture time
		check(EnumHasAllFlags(VelocitySwapchainDesc.Flags, Flags));
		check(VelocitySwapchainDesc.Format == Format);
		check(VelocitySwapchainDesc.NumMips == FMath::Max(NumMips, 1u));
		check(VelocitySwapchainDesc.NumSamples == NumSamples);
		
		OutTexture = VelocitySwapchain->GetTextureRef();
...
bool FOpenXRHMD::AllocateVelocityDepthTexture(uint32 Index, const FIntPoint& Size, uint8 Format, uint32 NumMips, ETextureCreateFlags Flags, FTextureRHIRef& OutTexture, uint32 NumSamples)
{
	check(IsInRenderingThread());

	FXRSwapChainPtr VelocityDepthSwapchain = nullptr;
	for (IOpenXRExtensionPlugin* Module : ExtensionPlugins)
	{
		if (FXRSwapChainPtr Swapchain = Module->GetVelocityDepthSwapchain_RenderThread())
		{
			if (Swapchain)
			{
				VelocityDepthSwapchain = Swapchain;
			}
		}
	}
	...
	{
		const FRHITexture* const VelocityDepthSwapchainTexture = VelocityDepthSwapchain->GetTexture2DArray() ? VelocityDepthSwapchain->GetTexture2DArray() : VelocityDepthSwapchain->GetTexture2D();
		const FRHITextureDesc& VelocityDepthSwapchainDesc = VelocityDepthSwapchainTexture->GetDesc();
		
		if (Size != VelocityDepthSwapchainDesc.Extent)
		{
			// We don't yet support velocity texture sizes other than the recommended size
			return false;
		}
		...
		// Sample count, mip count and size should be known at AllocateRenderTargetTexture time
		check(EnumHasAllFlags(VelocityDepthSwapchainDesc.Flags, Flags));
		check(VelocityDepthSwapchainDesc.Format == Format);
		check(VelocityDepthSwapchainDesc.NumMips == FMath::Max(NumMips, 1u));
		check(VelocityDepthSwapchainDesc.NumSamples == NumSamples);
		
		OutTexture = VelocityDepthSwapchain->GetTextureRef();
		return OutTexture.IsValid();
	}
...
{
	auto CreateSwapchain = [&](FRHITexture* Texture, ETextureCreateFlags Flags)
	{
		uint8 UnusedActualFormat = 0;
...
// BEGIN META SECTION - Add XR Cubemap Support
	const ETextureDimension TextureDimension = Layer.Desc.HasShape<FCubemapLayer>() ? ETextureDimension::TextureCube : ETextureDimension::Texture2D;

	if (Layer.NeedReallocateRightTexture(TextureDimension))
	{
		FRHITexture* Texture = Layer.Desc.Texture;
		Layer.RightEye.SetSwapchain(CreateSwapchain(Texture, Flags), Texture->GetSizeXY(), TextureDimension);
	}

	if (Layer.NeedReallocateLeftTexture(TextureDimension))
	{
		FRHITexture* Texture = Layer.Desc.LeftTexture;
		Layer.LeftEye.SetSwapchain(CreateSwapchain(Texture, Flags), Texture->GetSizeXY(), TextureDimension);
	}
// END META SECTION - Add XR Cubemap Support
}
...
// BEGIN META SECTION - Equirect2 
		(Layer.Desc.HasShape<FEquirectLayer>() && IsExtensionEnabled(XR_KHR_COMPOSITION_LAYER_EQUIRECT2_EXTENSION_NAME)) ||
// END META SECTION - Equirect2 
		Layer.Desc.HasShape<FQuadLayer>() ||
// BEGIN META SECTION - Add XR Cubemap Support
		(Layer.Desc.HasShape<FCubemapLayer>() && IsExtensionEnabled(XR_KHR_COMPOSITION_LAYER_CUBE_EXTENSION_NAME))
// BEGIN META SECTION - End XR Cubemap Support
		)
...
void FOpenXRHMD::UpdateLayerSwapchainTexture(const FOpenXRLayer& Layer, FRHICommandListImmediate& RHICmdList)
{
	...
	if (Layer.RightEye.Swapchain.IsValid() && Layer.Desc.Texture.IsValid())
	{
		...
		{
// BEGIN META SECTION - Add XR Cubemap Support
			FRHITexture* SrcTexture = Layer.Desc.Texture;
// END META SECTION - Add XR Cubemap Support
	...
	if (Layer.LeftEye.Swapchain.IsValid() && Layer.Desc.LeftTexture.IsValid())
	{
		...
		{
// BEGIN META SECTION - Add XR Cubemap Support
			FRHITexture* SrcTexture = Layer.Desc.LeftTexture;
// END META SECTION - Add XR Cubemap Support
...
FRHIRenderPassInfo FOpenXRHMD::SetupEmulatedLayersRenderPass(FRHICommandListImmediate& RHICmdList, const FSceneView& InView, TArray<IStereoLayers::FLayerDesc>& Layers, FTextureRHIRef RenderTarget, FDefaultStereoLayers_LayerRenderParams& OutRenderParams)
{
...
void FOpenXRHMD::OnBeginRendering_RenderThread(FRHICommandListImmediate& RHICmdList, FSceneViewFamily& ViewFamily)
{
	...
	for (IOpenXRExtensionPlugin* Module : ExtensionPlugins)
	{
		Module->OnBeginRenderingLate_RenderThread(Session, RHICmdList);
	}
	// END META SECTION - OpenXR Native Environment Depth
}
...
void FOpenXRHMD::PostRenderBasePassDeferred_RenderThread(FRDGBuilder& GraphBuilder, FSceneView& InView, const FRenderTargetBindingSlots& RenderTargets, TRDGUniformBufferRef<FSceneTextureUniformParameters> SceneTextures)
{
	for (IOpenXRExtensionPlugin* Module : ExtensionPlugins)
	{
...
void FOpenXRHMD::LocateViews(FPipelinedFrameState& PipelineState, bool ResizeViewsArray)
{
	...
	{
		// BEGIN META SECTION - OpenXR Native Environment Depth
		ViewInfo.next = Module->OnLocateViews(Session, ViewInfo.displayTime, SelectedViewConfigurationType, ViewInfo.next);
		// END META SECTION - OpenXR Native Environment Depth
	}
...
// BEGIN META SECTION - Add Depth Test 
			if (EnumHasAnyFlags(PipelinedLayerStateRendering.LayerStateFlags, EOpenXRLayerStateFlags::SubmitDepthLayer) && 
				IOpenXRHMDModule::Get().IsExtensionEnabled(XR_FB_COMPOSITION_LAYER_DEPTH_TEST_EXTENSION_NAME))
			{
...
// BEGIN META SECTION - OpenXR Native Environment Depth
bool FOpenXRHMD::FindEnvironmentDepthTexture_RenderThread(FTextureRHIRef& OutTexture, FTextureRHIRef& OutMinMaxTexture, FVector2f& OutDepthFactors, FMatrix44f OutScreenToDepthMatrices[2], FMatrix44f OutDepthViewProjMatrices[2])
{
	for (IOpenXRExtensionPlugin* Module : ExtensionPlugins)
	{
		if (Module->FindEnvironmentDepthTexture_RenderThread(OutTexture, OutMinMaxTexture, OutDepthFactors, OutScreenToDepthMatrices, OutDepthViewProjMatrices))
		{
			return true;
		}
	}
...
FIntRect FOpenXRHMD::GetFullFlatEyeRect_RenderThread(FTextureRHIRef EyeTexture) const
{
...
void FOpenXRHMD::CopyTexture_RenderThread(FRHICommandListImmediate& RHICmdList, FRHITexture* SrcTexture, FIntRect SrcRect, FRHITexture* DstTexture, FIntRect DstRect, 
...
// BEGIN META SECTION - Add XR Cubemap Support
	bool bIsCubeSource = DstTexture->GetDesc().Dimension == ETextureDimension::TextureCube;
	int TotalFaces = bIsCubeSource ? UE_ARRAY_COUNT(FaceToSampleMatrix) : 1;

// BEGIN META SECTION - Fix Layer Mip Copying
	const uint32 NumMips = SrcTexture->GetNumMips();
	for (uint32 MipIndex = 0; MipIndex < NumMips; ++MipIndex)
	{
		FRHITexture * ColorRT = DstTexture;
		FRHIRenderPassInfo RenderPassInfo(ColorRT, RTAction);
		RenderPassInfo.ColorRenderTargets[0].MipIndex = MipIndex;

		const uint32 MipViewportWidth = ViewportWidth >> MipIndex;
		const uint32 MipViewportHeight = ViewportHeight >> MipIndex;
// END META SECTION - Fix Layer Mip Copying

	for (int FaceIndex = 0; FaceIndex < TotalFaces; ++FaceIndex)
	{
		RenderPassInfo.ColorRenderTargets[0].ArraySlice = FaceIndex;
// END META SECTION - Add XR Cubemap Support
...
// BEGIN META SECTION - Fix Layer Mip Copying
			TShaderRef<FScreenPSMipLevel> ScreenPSMipLevel;
// END META SECTION - Fix Layer Mip Copying
// BEGIN META SECTION - Add XR Cubemap Support
			TShaderRef<FOculusOpenXRCubemapPS> CubemapPS;
// END META SECTION - Add XR Cubemap Support
			...
			if (FinalDstAccess == ERHIAccess::Present && RenderBridge.IsValid())
			{
				...
				// BEGIN META SECTION - OpenXR LDR Tonemap color space fix
				bool isTonemapEnabled = IsMobileTonemapSubpassEnabledInline(GetConfiguredShaderPlatform(), bIsMobileMultiViewEnabled, 1);
				if (!IsMobileHDR() && IsMobilePlatform(GetConfiguredShaderPlatform()) && IsSimulatedPlatform(GetConfiguredShaderPlatform()) && !isTonemapEnabled)
				// END META SECTION - OpenXR LDR Tonemap color space fix
				{
					bNeedsDisplayMapping = true;
...
// BEGIN META SECTION - Add XR Cubemap Support
			if (bIsCubeSource)
			{
				TShaderMapRef<FOculusOpenXRCubemapPS> CubemapPSRef(ShaderMap);
				CubemapPS = CubemapPSRef;
				PixelShader = CubemapPSRef;
			}
// END META SECTION - Add XR Cubemap Support
			else if (bNeedsDisplayMapping)
			{
			...
			else
			{
				...
				{
// BEGIN META SECTION - Fix Layer Mip Copying
					TShaderMapRef<FScreenPSMipLevel> ScreenPSRef(ShaderMap);
					ScreenPSMipLevel = ScreenPSRef;
// END META SECTION - Fix Layer Mip Copying
...
// BEGIN META SECTION - Fix Layer Mip Copying
			if (ScreenPSMipLevel.IsValid())
			{
				FRHISamplerState* PixelSampler = bSameSize ? TStaticSamplerState<SF_Point>::GetRHI() : TStaticSamplerState<SF_Bilinear>::GetRHI();
				SetShaderParametersLegacyPS(RHICmdList, ScreenPSMipLevel, PixelSampler, SrcTexture, MipIndex);
			}
// END META SECTION - Fix Layer Mip Copying
			else if (ScreenPS.IsValid())
			{
				FRHISamplerState* PixelSampler = bSameSize ? TStaticSamplerState<SF_Point>::GetRHI() : TStaticSamplerState<SF_Bilinear>::GetRHI();
...
// BEGIN META SECTION - Add XR Cubemap Support
			else if (CubemapPS.IsValid())
			{
				check(FaceIndex < UE_ARRAY_COUNT(FaceToSampleMatrix));
				FRHISamplerState* PixelSampler = bSameSize ? TStaticSamplerState<SF_Point>::GetRHI() : TStaticSamplerState<SF_Bilinear>::GetRHI();
				SetShaderParametersLegacyPS(RHICmdList, CubemapPS, PixelSampler, SrcTexture, FaceToSampleMatrix[FaceIndex]);
			}
// END META SECTION - Add XR Cubemap Support
			...
			RendererModule->DrawRectangle(
				...
				0, 0,
// BEGIN META SECTION - Fix Layer Mip Copying
				MipViewportWidth, MipViewportHeight,
// END META SECTION - Fix Layer Mip Copying
				U, V,
				USize, VSize,
				TargetSize,
...
void FOpenXRHMD::CopyTexture_RenderThread(FRHICommandListImmediate& RHICmdList, FRHITexture* SrcTexture, FIntRect SrcRect, const FXRSwapChainPtr& DstSwapChain, FIntRect DstRect, bool bClearBlack, ETextureCopyBlendModifier SrcTextureCopyModifier) const
{
// BEGIN META SECTION - Add XR Cubemap Support
	if (bIsAcquireOnAnyThreadSupported)
	{
...
void FOpenXRHMD::CopyTexture_RenderThread(FRHICommandListImmediate& RHICmdList, FRHITexture* SrcTexture, FIntRect SrcRect, FRHITexture* DstTexture, FIntRect DstRect, bool bClearBlack, bool bNoAlpha) const
{
...
void FOpenXRHMD::RenderTexture_RenderThread(class FRHICommandListImmediate& RHICmdList, class FRHITexture* BackBuffer, class FRHITexture* SrcTexture, FVector2D WindowSize) const
{
	if (SpectatorScreenController)
	{
		...
		const FTextureRHIRef LayersTexture = bShouldPassLayersTexture ? PipelinedLayerStateRendering.EmulatedLayerState.EmulationSwapchain->GetTextureRef() : nullptr;
...
{
	const uint32_t LayerId = TStereoLayerManager::CreateLayer(InLayerDesc);
...
// BEGIN META SECTION - OpenXR Native Environment Depth
int64_t FOpenXRHMD::GetPredictedDisplayTimeForThread() const
{
	return GetPipelinedFrameStateForThread().GetFrameState().FrameState.predictedDisplayTime;
}
// END META SECTION - OpenXR Native Environment Depth
