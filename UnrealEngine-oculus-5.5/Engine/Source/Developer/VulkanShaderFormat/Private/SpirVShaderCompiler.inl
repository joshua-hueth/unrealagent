...
// BEGIN META SECTION - Emulated Uniform Buffers
		, bUseEmulatedUniformBuffers(InInput.Environment.CompilerFlags.Contains(CFLAG_UseEmulatedUB))
// END META SECTION - Emulated Uniform Buffers
	{
...
// BEGIN META SECTION - Emulated Uniform Buffers
	const bool bUseEmulatedUniformBuffers;
// END META SECTION - Emulated Uniform Buffers
...
// BEGIN META SECTION - Emulated Uniform Buffers
static bool BuildShaderOutput(
	FOptimizedUniforms&			OptimizedUniforms,
// END META SECTION - Emulated Uniform Buffers
...
// BEGIN META SECTION - Emulated Uniform Buffers
	// Write out emulated UB metadata
	{
		check(OptimizedUniforms.EmulatedUBCopies.Num() == 0 || InternalState.bUseEmulatedUniformBuffers);
		uint16 CachedParameter = 0xFFFF;
		TOptional<FParameterAllocation> Parameter;
		for (const FEmulatedUBCopy& PackedUBCopy : OptimizedUniforms.EmulatedUBCopies)
		{
	        if (PackedUBCopy.FromBuffer != CachedParameter)
	        {
                CachedParameter = PackedUBCopy.FromBuffer;
                Parameter = ShaderOutput.ParameterMap.FindParameterAllocation(OptimizedUniforms.EmulatedUBs[PackedUBCopy.FromBuffer]);
	        }
	        check(Parameter.IsSet()); // Emulated UBs must have parameter allocations
	        FVulkanShaderHeader::FEmulatedUBCopy CopyInfo;
	        CopyInfo.SourceUBIndex = Parameter->BufferIndex;
	        CopyInfo.SourceOffsetInFloats = PackedUBCopy.FromOffsetFloats;
	        CopyInfo.DestOffsetInFloats = PackedUBCopy.ToOffsetFloats;
	        CopyInfo.SizeInFloats = PackedUBCopy.SizeFloats;

	        Header.EmulatedUBCopies.Add(CopyInfo);
		}
		Header.EmulatedUBCopies.Sort([](const FVulkanShaderHeader::FEmulatedUBCopy& Lhs, const FVulkanShaderHeader::FEmulatedUBCopy& Rhs) -> bool
		{
	        #define SORT_BY_FIELD(Field, Op) \
                if (Lhs.Field != Rhs.Field) \
                { \
                    return Lhs.Field Op Rhs.Field; \
                }

	        SORT_BY_FIELD(SourceUBIndex, <)       // Primary sort by source UB
	        SORT_BY_FIELD(SizeInFloats, >)        // Secondary sort by copy size, to help the loop predictor
	        SORT_BY_FIELD(DestOffsetInFloats, <)  // Tertiary sort by destination offset, to help the prefetcher

	        // Otherwise keep initial ordering
	        return false;
	        #undef SORT_BY_FIELD
		});
		...
		{
	        TMap<uint32, FVulkanShaderHeader::FEmulatedUBCopyRange> PackedUBGlobalCopiesRanges;
	        int32 MaxDestUBIndex = -1;
	        {
                // Verify table is sorted
                int32 PrevSourceUB = -1;
                int32 Index = 0;
                for (auto& Copy : Header.EmulatedUBCopies)
                {
                    if (PrevSourceUB < (int32)Copy.SourceUBIndex)
                    {
                        PrevSourceUB = Copy.SourceUBIndex;
                        MaxDestUBIndex = FMath::Max(MaxDestUBIndex, (int32)Copy.SourceUBIndex);
                        FVulkanShaderHeader::FEmulatedUBCopyRange InitialRange;
                        InitialRange.Start = Index;
                        InitialRange.Count = 1;
                        PackedUBGlobalCopiesRanges.Add(Copy.SourceUBIndex) = InitialRange;
                    }
                    else if (PrevSourceUB == Copy.SourceUBIndex)
                    {
                        ++PackedUBGlobalCopiesRanges.FindChecked(Copy.SourceUBIndex).Count;
                    }
                    else
                    {
                        // Internal error, means the above sort didn't work.
                        check(0);
                    }
                    ++Index;
                }
	        ...
	        Header.EmulatedUBCopyRanges.AddZeroed(MaxDestUBIndex + 1); // +1 because it's an inclusive max index
	        for (int32 Index = 0; Index <= MaxDestUBIndex; ++Index)
	        {
                if (FVulkanShaderHeader::FEmulatedUBCopyRange* Found = PackedUBGlobalCopiesRanges.Find(Index))
                {
                    Header.EmulatedUBCopyRanges[Index] = *Found;
                }
	        }
		}
	}
...
// BEGIN META SECTION - Emulated Uniform Buffers
	FOptimizedUniforms OptimizedUniforms;
	// For builds using RootParametersStructure, don't optimize globals.
	if (!InternalState.UseRootParametersStructure() &&
		!OptimizeGlobalUniforms(InternalState.Input, CompilerContext, SerializedOutput.Spirv, OptimizedUniforms, InternalState.bDebugDump, InternalState.bUseEmulatedUniformBuffers))
	{
		return false;
	}
// END META SECTION - Emulated Uniform Buffers
		...
		auto AddReflectionInfos = [&](TArray<SpvReflectDescriptorBinding*>& BindingArray, const VkDescriptorType DescriptorType, int32 BindingTypeCount, bool bIsPackedUniformBuffer=false)
		{
			for (const SpvReflectDescriptorBinding* Binding : BindingArray)
			{
				...
				case VK_DESCRIPTOR_TYPE_SAMPLER:
					...
					//HandleReflectedShaderSampler(ResourceName, BindingTypeCount, ReflectionSlot, 1, Output);
					break;
...
// BEGIN META SECTION - Emulated Uniform Buffers
							...
							for (FPackedGlobal Global : OptimizedUniforms.PackedGlobals)
							{
								FString MemberName(Global.Name);
								...
								if (!InternalState.UseRootParametersStructure() || BindlessParameterType != EShaderParameterType::LooseData)
								{
									...
									check(Global.Offset + Global.Size <= SerializedOutput.Header.PackedGlobalsSize);
...
// BEGIN META SECTION - Emulated Uniform Buffers
		int32 UBOBindings = AddReflectionInfos(Bindings.UniformBuffers, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, GlobalUBCount);
		for (const FString& EmulatedUBName : OptimizedUniforms.EmulatedUBs)
		{
			int32 ReflectionSlot = UsedUniformBufferSlots.FindAndSetFirstZeroBit();
			HandleReflectedUniformBuffer(EmulatedUBName, ReflectionSlot, UBOBindings, 0, Output);

			FVulkanShaderHeader::FUniformBufferInfo& UniformBufferInfo = SerializedOutput.Header.UniformBufferInfos.AddZeroed_GetRef();
			UniformBufferInfo.LayoutHash = GetUBLayoutHash(InternalState.Input, EmulatedUBName);
			++UBOBindings;
		}
// END META SECTION - Emulated Uniform Buffers
...
// BEGIN META SECTION - Emulated Uniform Buffers
	bool bSuccess = BuildShaderOutput(
		OptimizedUniforms,
// END META SECTION - Emulated Uniform Buffers
		SerializedOutput,
		Output,
...
// BEGIN META SECTION - Emulated Uniform Buffers
	const FShaderSource::FViewType PreprocessedShader,
// END META SECTION - Emulated Uniform Buffers
...
// BEGIN META SECTION - Emulated Uniform Buffers
	const FString HLSLFilename = FPaths::GetBaseFilename(USFFilename) + TEXT(".DXC.hlsl");
// END META SECTION - Emulated Uniform Buffers
...
// BEGIN META SECTION - Emulated Uniform Buffers
	UE::ShaderCompilerCommon::FDebugShaderDataOptions DebugOptions;
	DebugOptions.OverrideBaseFilename = *HLSLFilename;
	FString SourceString(PreprocessedShader);
	UE::ShaderCompilerCommon::DumpDebugShaderData(InternalState.Input, SourceString, DebugOptions);
// END META SECTION - Emulated Uniform Buffers
}
...
)
{
	...
	{
// BEGIN META SECTION - Emulated Uniform Buffers
		SpirvCreateDXCCompileBatchFiles(PreprocessedShader, CompilerContext, InternalState, Options);
// END META SECTION - Emulated Uniform Buffers
	}
