	...
	class UEDeployAndroid : UEBuildDeploy, IAndroidDeploy
	{
		...
		private string GetSdkApiLevel(AndroidToolChain ToolChain)
		{
			if (CachedSDKLevel == null)
			{
				...
				if (SDKLevelInt == 0)
				{
					Logger.LogWarning("Unable to parse SDK API level '{SDKLevel}', it is expected to be of the form 'android-NN'", SDKLevel);
				}
		...
		private bool CopyAPKAndReplaceSO(ILogger Logger, string SourceAPK, string DestAPK, string SourceSOFile, string DestSOFile)
		{
			...
			while (true)
			{
				...
				{
// BEGIN META SECTION - Bypass gradle packaging fix
					// Skip 'CompressedSize' bytes to seek next file header
					SourceReader.BaseStream.Position += CompressedSize;
// END META SECTION - Bypass gradle packaging fix
		...
		private void MakeApk(AndroidToolChain ToolChain, string ProjectName, TargetType InTargetType, string ProjectDirectory, string OutputPath, string EngineDirectory, bool bForDistribution, string CookFlavor,
			UnrealTargetConfiguration Configuration, bool bMakeSeparateApks, bool bIncrementalPackage, bool bDisallowPackagingDataInApk, bool bDisallowExternalFilesDir, bool bSkipGradleBuild, bool bIsArchive, bool bIsFromUAT, bool bSkipUECommandLineMatch = false)
		{
			Logger.LogInformation("");
			...
			if (bBuildSettingsMatch)
			{
				// BEGIN META SECTION - Iterative Cook-on-the-Fly
				if (bSkipUECommandLineMatch)
				{
					// Skip matching UECommandLine.txt
					// Most of the UECommandLine args should be irrelevant to the APK build at this step (e.g. Session ID)
				}
				else
				{
				// END META SECTION - Iterative Cook-on-the-Fly
...
// BEGIN META SECTION - Bypass gradle packaging fix
					int MinSDKVersion = 0;
					GetMinTargetSDKVersions(ToolChain, Arch, UPL, NDKArch, bEnableBundle, out MinSDKVersion, out _, out _);
// END META SECTION - Bypass gradle packaging fix
...
// BEGIN META SECTION - Bypass gradle packaging fix
						StartInfo.ArgumentList.Add("--min-sdk-version");
						StartInfo.ArgumentList.Add(MinSDKVersion.ToString());
// END META SECTION - Bypass gradle packaging fix
					...
					else
					{
						...
						StartInfo.ArgumentList.Add("-c");
// BEGIN META SECTION - Bypass gradle packaging fix
						StartInfo.ArgumentList.Add("\"" + APKSignerExecutable + "\" sign--ks \"" + KeyStore + "\" --ks-pass pass:" + KeyStorePassword + " --ks-key-alias " + KeyAlias + "--min-sdk-version " + MinSDKVersion.ToString() + " --key-pass pass:" + KeyPassword + " --out \"" + DestApkName + "2\" \"" + TempAPK + "\"");
// END META SECTION - Bypass gradle packaging fix
					}
					StartInfo.UseShellExecute = false;
					StartInfo.WindowStyle = ProcessWindowStyle.Minimized;
...
//				LocalProperties.AppendLine(string.Format("ndk.dir={0}", Environment.GetEnvironmentVariable("NDKROOT")!.Replace("\\", "/")));
		...
		// BEGIN META SECTION - Iterative Cook-on-the-Fly
			bool bSkipUECommandLineMatch = false
		// END META SECTION - Iterative Cook-on-the-Fly
		)
		{
			...
			MakeApk(ToolChain, ProjectName, Type, ProjectDirectory.FullName, ExecutablePath, EngineDirectory, bForDistribution: bForDistribution, CookFlavor: CookFlavor, Configuration: Configuration,
				...
				bSkipGradleBuild: bSkipGradleBuild, bIsArchive: bIsArchive, bIsFromUAT: true, bSkipUECommandLineMatch);
			return true;
		}
