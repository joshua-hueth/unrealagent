...
class URendererSettings : public UDeveloperSettings
{
	...
	*/
	...
	UPROPERTY(config, EditAnywhere, Category = VR, meta = (
		ConsoleVariable = "r.Vulkan.UseEmulatedUBs", DisplayName = "Enable Emulated Uniform Buffers",
		EditCondition="!bMobileSupportGPUScene",
		ToolTip = "Optimizes performance by consolidating constant buffers into a single global uniform buffer for improved shader compiler optimization.",
		ConfigRestartRequired = true))
	...
	*/
	...
	UPROPERTY(config, EditAnywhere, Category = Mobile, meta = (
		EditCondition = "!bMobileUniformLocalLights",
		ConsoleVariable = "r.Mobile.PackLightGridLightDataToUBO.Enable",
		DisplayName = "Enable packing LightGrid light data to UBO",
		ToolTip = "When Forward+ LightGrid is used, enabling this will improve performance by packing LightGrid light data to UBO. Change GMaxForwardMovableLights (default to 32) per game's scene to utilize UBO efficiently.",
		ConfigRestartRequired = true))
	uint32 bMobilePackLightGridLightDataToUBO : 1;
	// END META SECTION - LightGrid LightDataUBO
	
	// BEGIN META SECTION - Bound Uniform Local Lights
	UPROPERTY(config, EditAnywhere, Category = Mobile, meta = (
		ConsoleVariable = "r.Mobile.UniformLocalLights.Enable",
		DisplayName = "Enable uniform local lights support on mobile forward",
		ToolTip = "Instead of using Forward+ LightGrid, bind lights to every draw in uniforms, classic Forward Rendering style. This can be a performance improvement on some mobile devices.",
		ConfigRestartRequired = true))
	uint32 bMobileUniformLocalLights : 1;
