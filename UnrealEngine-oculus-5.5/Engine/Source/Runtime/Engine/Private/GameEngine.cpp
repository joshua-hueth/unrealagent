...
// BEGIN META SECTION - VR MultiPlayer
	const FText WindowTitleComponent = WindowTitleOverride.IsEmpty() ? NSLOCTEXT("UnrealEd", "GameWindowTitle", "{GameName}, {ExtraInfo}") : WindowTitleOverride;
...
// BEGIN META SECTION - VR MultiPlayer
#if WITH_EDITOR
	// In case of MultiPlayer, show InstanceNumber on the title.
	int32 InInstanceNum = -1;
	FParse::Value(FCommandLine::Get(), TEXT("InInstanceNum="), InInstanceNum);
	const FText ExtraInfo = (InInstanceNum <= 0 ? FText() : FText::Format(FText::FromString(TEXT("Standalone {0}")), FText::AsNumber(InInstanceNum)));
	Args.Add( TEXT("ExtraInfo"), ExtraInfo);
#else
	Args.Add(TEXT("ExtraInfo"), FText());
#endif
// END META SECTION - VR MultiPlayer
