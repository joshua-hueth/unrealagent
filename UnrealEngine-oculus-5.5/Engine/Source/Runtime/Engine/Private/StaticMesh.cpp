...
void UStaticMesh::PostLoad()
{
	...
	if (FStaticMeshCompilingManager::Get().IsAsyncCompilationAllowed(this))
	{
		...
		// We CANNOT wait for AssetUserData::PostLoad() to be called later by framework which is too late.
		// Let's explicitly call AssetUserData::FinishCompilation() in case that AssetUserData::CacheDerivedData() 
		// or some other functions need safely do something else (e.g. build Occluder mesh data with RenderData).
		for (int32 DataIdx = 0; DataIdx < AssetUserData.Num(); DataIdx++)
		{
			UAssetUserData* Datum = AssetUserData[DataIdx];
			if (Datum != NULL)
			{
				Datum->FinishCompilation();
			}
		}
		// END META SECTION - Software Occlusion
