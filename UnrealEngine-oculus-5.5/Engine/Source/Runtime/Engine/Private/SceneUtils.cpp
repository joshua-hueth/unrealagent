...
// BEGIN META SECTION 5.5 intead of checking bMultiViewRendering == true to turn on tonemap subpass, we should just check IsMobileHDR, HDR = TonemapSubpass on.
ENGINE_API bool IsMobileTonemapSubpassEnabled(EShaderPlatform Platform, bool)
// END META SECTION
{
	static auto* MobileTonemapSubpassPathCvar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.Mobile.TonemapSubpass"));
	// BEGIN META SECTION - LDR Tonemap Subpass + 5.5 bMultiViewRendering does not always use TonemapSubpass (aka LDR w/o tonemap ). so checking IsMobileHDR() instead
