	...
	FVertexStreamList& Streams)
{
	...
	const bool bMobileUsesGPUScene = IsAndroidPlatform(GMaxRHIShaderPlatform) && MobileSupportsGPUScene();
...
void FInstancedStaticMeshSceneProxy::GetLocalToWorldArray(TArray<FMatrix>& LocalToWorlds) const
{
	const FInstanceSceneDataBuffers* InstanceSceneDataBuffers = GetInstanceSceneDataBuffers();
	if (InstanceSceneDataBuffers)
	{
		for (int32 InstanceIndex = 0; InstanceIndex < InstanceSceneDataBuffers->GetNumInstances(); ++InstanceIndex)
		{
			LocalToWorlds.Add(InstanceSceneDataBuffers->GetInstanceToWorld(InstanceIndex));
		}
	}
	check(GetInstanceDataHeader().NumInstances == LocalToWorlds.Num());
}
