...
// Begin view uniform macro handlers for CopyIntoInstancedViewParameters
...
void InstancedViewParametersUtils::CopyIntoInstancedViewParameters(FInstancedViewUniformShaderParameters& InstancedViewParameters, const FViewUniformShaderParameters& ViewParameters, uint32 CopyViewId)
{
	VIEW_UNIFORM_BUFFER_MEMBER_TABLE

	if (CopyViewId == 1)
	{
		FLargeWorldRenderPosition  LeftEyePVT(FVector3d(InstancedViewParameters.ViewTilePosition[0]), FVector3d(InstancedViewParameters.RelativePreViewTranslationTO[0]));
		FLargeWorldRenderPosition RightEyePVT(FVector3d(InstancedViewParameters.ViewTilePosition[1]), FVector3d(InstancedViewParameters.RelativePreViewTranslationTO[1]));
		InstancedViewParameters.PreViewTranslationEyeAdjustment[1] = FVector3f(RightEyePVT.GetAbsolute() - LeftEyePVT.GetAbsolute());
	}
}
...
// BEGIN META SECTION - Scene Flag for Multiview Shaders
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	, bHasMobileMultiviewShaders(false)
PRAGMA_ENABLE_DEPRECATION_WARNINGS
// END META SECTION - Scene Flag for Multiview Shaders
	...
	, FeatureLevel(InitOptions.ViewFamily ? InitOptions.ViewFamily->GetFeatureLevel() : GMaxRHIFeatureLevel)
{
	...
	// Query instanced stereo and multi-view state
	{
		...
		// BEGIN META SECTION - Scene Flag for Multiview Shaders
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		bHasMobileMultiviewShaders = Aspects.IsMobileMultiViewEnabled();
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		// END META SECTION - Scene Flag for Multiview Shaders
		// BEGIN META SECTION - OpenXR AppSpaceWarp
		bIncludeStaticInVelocityPass = Family ? Family->bRenderStereoVelocity : false;
	...
	const FSetupViewUniformParametersInputs& Inputs)
{
	...
	// BEGIN META SECTION - LightGrid Optimization
	ViewUniformShaderParameters.PreViewTranslationEyeAdjustment = FVector3f::ZeroVector;
	// END META SECTION - LightGrid Optimization
