...
void FStaticResolutionFractionHeuristic::PullViewFamilyRenderingSettings(const FSceneViewFamily& ViewFamily)
{
	...
	// BEGIN META SECTION - VR ScreenPercentage Fix
	// Even though FUserSettings::PullRunTimeRenderingSettings usually sets the screen percentage mode,
	// the StereoRendering flag in the ViewFamily can be set by a view family extension for each view
	// family so we need set the screen percentage mode for VR here again if needed
	if ((ViewFamily.EngineShowFlags.StereoRendering != 0) || (ViewFamily.EngineShowFlags.VREditing != 0))
	{
		Settings.Mode = static_cast<EScreenPercentageMode>(FMath::Clamp(CVarScreenPercentageDefaultVRMode.GetValueOnGameThread(), 0, 2));
	}
	// END META SECTION - VR ScreenPercentage Fix
