...
// BEGIN META SECTION - Time Dilation Audio fix
...
FAutoConsoleVariableRef CVaUseTimeDilationToFixPlayDuration(
	...
	ECVF_Default);
// END META SECTION - Time Dilation Audio fix
...
private:
			...
			UObject* PlaybackContext) const
	{
		...
		if (UseTimeDilationToFixPlayDuration)
		{
			...
			if (World != nullptr)
			{
				InverseTimeDilation = (1.0f / World->GetWorldSettings()->GetEffectiveTimeDilation());
		...
		// BEGIN META SECTION - Time Dilation Audio fix
		float AudioTime = (Context.GetTime() / Context.GetFrameRate()) * InverseTimeDilation
		...
		if (AudioTime >= 0.f && Sound)
		{
			...
			UWorld* World = PlaybackContext ? PlaybackContext->GetWorld() : nullptr;
			if (UseTimeDilationToFixPlayDuration && World != nullptr)
			{
				...
				if (!EvaluationData.LastAudioTime.Contains(&AudioComponent))
				{
					// Add the audio components to trackers
					EvaluationData.LastAudioTime.Add(&AudioComponent);
					EvaluationData.LastContextTime.Add(&AudioComponent);
					// Store current audio time and current context time
					EvaluationData.LastAudioTime[&AudioComponent] = AudioTime;
					EvaluationData.LastContextTime[&AudioComponent] = Context.GetTime() / Context.GetFrameRate();
				}
				else
				{
					// Get previous AudioTime
					AudioTime = EvaluationData.LastAudioTime[&AudioComponent] < AudioTime ? EvaluationData.LastAudioTime[&AudioComponent] : AudioTime;
					float CurrContextTime = (Context.GetTime() / Context.GetFrameRate());
					float PrevContextTime = EvaluationData.LastContextTime[&AudioComponent];
					...
					EvaluationData.LastAudioTime[&AudioComponent] = AudioTime;
					EvaluationData.LastContextTime[&AudioComponent] = CurrContextTime;
				}
			}
			// END META SECTION - Time Dilation Audio fix
