...
// BEGIN META SECTION - Emulated Uniform Buffers
	if (IsOpenGLPlatform(Platform) && IsMobilePlatform(Platform))
	{
		// DXC only supports emulated uniform buffers on GLES
		return true;
	}
	else if (IsVulkanPlatform(Platform))
	{
		static const auto CVarMobileSupportGPUScene = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.Mobile.SupportGPUScene"));
		bool bMobileSupportsGPUScene = CVarMobileSupportGPUScene->GetValueOnAnyThread() != 0;

		static const auto CVarEmulatedUniformBuffers = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.Vulkan.UseEmulatedUBs"));
		bool bEmulatedUniformBuffers = CVarEmulatedUniformBuffers->GetValueOnAnyThread() != 0;

		return bEmulatedUniformBuffers && !bMobileSupportsGPUScene;
	}
...
void ShaderMapAppendKey(EShaderPlatform Platform, FShaderKeyGenerator& KeyGen)
{
	...
	{
		KeyGen.AppendSeparator();
		KeyGen.Append(TEXT("ASW"));
	}

	// BEGIN META SECTION - LightGrid LightDataUBO
	{
		static const auto CVarPackLightGridLightDataToUBO = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.Mobile.PackLightGridLightDataToUBO.Enable"));
		if(CVarPackLightGridLightDataToUBO && CVarPackLightGridLightDataToUBO->GetValueOnAnyThread() != 0)
		{
			KeyGen.AppendSeparator();
			KeyGen.Append(TEXT("LIGHTGRIDUBO"));
		}
	}
