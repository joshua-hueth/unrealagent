...
void FRDGBuilder::ExecuteSerialPass(FRHIComputeCommandList& RHICmdListPass, FRDGPass* Pass)
{
	...
	if(EnumHasAnyFlags(Pass->Flags, ERDGPassFlags::EnableLateLatching))
	{
		RDGRequestMobileLateLatching.Broadcast(*this);
	}
...
void FRDGBuilder::AddLastTextureTransition(FRDGTexture* Texture)
{
	...
	for (FRDGSubresourceState*& State : Texture->State)
	{
		if (!State)
		{
			...
			{
				const FRDGPassHandle AcquirePass = Texture->AcquirePass.IsValid() ? Texture->FirstPass : GetProloguePassHandle();
