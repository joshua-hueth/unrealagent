...
RENDERCORE_API bool MobileUniformLocalLightsEnable(const FStaticShaderPlatform Platform)
{
	static FShaderPlatformCachedIniValue<int32> MobileUniformLocalLightsEnableIniValue(TEXT("r.Mobile.UniformLocalLights.Enable"));
	return MobileUniformLocalLightsEnableIniValue.Get(Platform) > 0 && !IsMobileDeferredShadingEnabled(Platform);
}

RENDERCORE_API int32 MobileUniformLocalLightsNumUnrolledLights(const FStaticShaderPlatform Platform)
{
	static FShaderPlatformCachedIniValue<int32> MobileUniformLocalLightsNumUnrolledLightsIniValue(TEXT("r.Mobile.UniformLocalLights.NumUnrolledLights"));
	return FMath::Clamp(MobileUniformLocalLightsNumUnrolledLightsIniValue.Get(Platform), 0, MOBILE_FORWARD_UNIFORM_LOCAL_LIGHTS_UNROLLED_MAX);
}

RENDERCORE_API int32 MobileUniformLocalLightsMax(const FStaticShaderPlatform Platform)
{
	static FShaderPlatformCachedIniValue<int32> MobileUniformLocalLightsMaxIniValue(TEXT("r.Mobile.UniformLocalLights.MaxLights"));
	return FMath::Clamp(MobileUniformLocalLightsMaxIniValue.Get(Platform), 0, MOBILE_FORWARD_UNIFORM_LOCAL_LIGHTS_MAX);
}
// END META SECTION - Bound Uniform Local Lights

// BEGIN META SECTION - LightGrid LightDataUBO
RENDERCORE_API bool MobilePackLightGridLightDataToUBOEnable(const FStaticShaderPlatform Platform)
{
	static FShaderPlatformCachedIniValue<int32> MobilePackLightGridLightDataToUBOEnableIniValue(TEXT("r.Mobile.PackLightGridLightDataToUBO.Enable"));
	return (MobilePackLightGridLightDataToUBOEnableIniValue.Get(Platform) > 0 &&
		!IsMobileDeferredShadingEnabled(Platform) &&
		!MobileUniformLocalLightsEnable(Platform)); //Don't enable if UniformLocalLight is enabled
}
...
bool UseGPUScene(const FStaticShaderPlatform Platform, const FStaticFeatureLevel FeatureLevel)
{
	...
	{
		// BEGIN META SECTION - Android Preview GPU Scene workaround
#if WITH_EDITOR
		if (!IsAndroidPlatform(Platform) && MobileSupportsGPUScene())
		{
			static bool bShouldWarnGPUSceneVKPreview = true;
			UE_CLOG(bShouldWarnGPUSceneVKPreview, LogRendererCore, Warning, TEXT("Mobile GPUScene support disabled for feature level ES3_1 (Mobile Renderer)."));
			bShouldWarnGPUSceneVKPreview = false;
		}
#endif
		return IsAndroidPlatform(Platform) && MobileSupportsGPUScene();
		// END META SECTION - Android Preview GPU Scene workaround
	}
	...
	// BEGIN META SECTION - Emulated Uniform Buffers
		&& !IsUsingEmulatedUniformBuffers(Platform)
