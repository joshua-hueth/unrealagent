...
class UE_DEPRECATED(5.5, "Use FOculusOpenXRCubemapPS() instead.") FOculusCubemapPS : public FGlobalShader
...
// BEGIN META SECTION - Add XR Cubemap Support
class FOculusOpenXRCubemapPS : public FGlobalShader
{
	DECLARE_EXPORTED_SHADER_TYPE(FOculusOpenXRCubemapPS, Global, RENDERCORE_API);
public:

	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters) { return true; }

	FOculusOpenXRCubemapPS(const ShaderMetaType::CompiledShaderInitializerType& Initializer) :
		FGlobalShader(Initializer)
	{
		InTexture.Bind(Initializer.ParameterMap, TEXT("InTextureCube"), SPF_Mandatory);
		InTextureSampler.Bind(Initializer.ParameterMap, TEXT("InTextureSampler"));
