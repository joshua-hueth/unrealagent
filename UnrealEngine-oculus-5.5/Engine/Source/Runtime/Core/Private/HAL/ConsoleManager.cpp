...
static TAutoConsoleVariable<int32> CVarMobileUniformLocalLightsEnable(
	TEXT("r.Mobile.UniformLocalLights.Enable"),
	0,
	TEXT("0: Use the Forward+ light grid to render dynamic point and spot lights\n")
	TEXT("1: Bind lights to every draw in uniforms, classic Forward Rendering style. This can be a performance improvement on some mobile devices."),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarMobileUniformLocalLightsNumUnrolledLights(
	TEXT("r.Mobile.UniformLocalLights.NumUnrolledLights"),
	0,
	TEXT("To improve performance, specialized shader permutations will be generated for specific numbers of\n")
	TEXT("dynamic lights. This adjusts how many specialized permutations are made before falling back to a loop.\n")
	TEXT("Set to 0 to disable specialized permutations. Set to the same value as r.Mobile.UniformLocalLights.MaxLights\n")
	TEXT("to disable the loop permutation and only use unrolled shaders.\n")
	TEXT("No more than 4 lights can be unrolled, to limit the number of shader permutations.\n")
	TEXT("Default: 0"),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarMobileUniformLocalLightsMax(
	TEXT("r.Mobile.UniformLocalLights.MaxLights"),
	8,
	TEXT("The maximum number of dynamic point lights to support per draw on mobile devices, when UniformLocalLights are enabled.\n")
	TEXT("Setting this to 0 for games which do not require dynamic point lights will reduce the number of shaders generated."),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);
// END META SECTION - Bound Uniform Local Lights

// BEGIN META SECTION - LightGrid LightDataUBO
static TAutoConsoleVariable<int32> CVarMobilePackLightGridLightDataToUBOEnable(
	TEXT("r.Mobile.PackLightGridLightDataToUBO.Enable"),
	1,
	TEXT("0: Use non-UBO buffer to hold lightgrid light data\n")
	TEXT("1: Pack LightGrid light data to UBO."),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);
...
static TAutoConsoleVariable<int32> CVarVulkanUseEmulatedUBs(
	TEXT("r.Vulkan.UseEmulatedUBs"),
	0,
	TEXT("0: Use real uniform buffers for all vulkan targets\n")
	TEXT("1: Emulate uniform buffers on all vulkan targets\n")
	TEXT("This CVar is incompatible with several engine features. Enabling any of the following features will disable emulated uniform buffers:\n")
	TEXT("    GPUScene"),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);
