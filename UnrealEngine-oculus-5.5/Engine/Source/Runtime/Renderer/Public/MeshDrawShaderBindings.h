...
// BEGIN META SECTION - Bound Uniform Local Lights
		Add(Parameter, &Value, sizeof(ParameterType));
	}

	void Add(FShaderParameter Parameter, const void* Value, int32 DataSizeBytes, bool bValidateDataSize = true)
	{
				...
				if (LooseParameterBuffer.BaseIndex == Parameter.GetBufferIndex())
				{
					...
					for (int32 LooseParameterIndex = 0; LooseParameterIndex < Parameters.Num(); LooseParameterIndex++)
					{
						...
						if (Parameter.GetBaseIndex() == LooseParameter.BaseIndex)
						{
							...
							// BEGIN META SECTION - Bound Uniform Local Lights
							ensureMsgf(!bValidateDataSize || DataSizeBytes == Parameter.GetNumBytes(), TEXT("Attempted to set fewer bytes than the shader required.  Setting %u bytes on loose parameter at BaseIndex %u, Size %u.  This can cause GPU hangs, depending on usage."), DataSizeBytes, Parameter.GetBaseIndex(), Parameter.GetNumBytes());
							const int32 NumBytesToSet = FMath::Min<int32>(DataSizeBytes, Parameter.GetNumBytes());
							FMemory::Memcpy(LooseDataOffset, Value, NumBytesToSet);
							const int32 NumBytesToClear = Parameter.GetNumBytes() - NumBytesToSet;
							// END META SECTION - Bound Uniform Local Lights
