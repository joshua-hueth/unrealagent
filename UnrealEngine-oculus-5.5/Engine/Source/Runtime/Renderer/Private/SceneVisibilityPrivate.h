...
private:
	...
	enum class EVisibilityMergeStep
	{
		// Visibility is merging after the frustum cull step
		FrustumCull,

		// Visibility is merging after the occlusion cull step
		OcclusionCull,
	};
	
	// In some cases with multiview or instanced stereo, the secondary view doesn't submit occlusion queries and the 
	...
	void MergeSecondaryViewVisibility(EVisibilityMergeStep VisibilityStep) const;
...
public:
	...
	// BEGIN META SECTION - GPU Occlusion Query Optimizations	
	static bool ShouldSubmitQueriesForView(const FViewInfo& InView);
	// END META SECTION - GPU Occlusion Query Optimizations
