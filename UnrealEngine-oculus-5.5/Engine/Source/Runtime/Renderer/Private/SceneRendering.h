...
// The maximum number of dynamic LightGrid lights to pack into UBO on mobile devices when r.Mobile.PackLightGridLightDataToUBO.Enable is 1.
// Note that increasing GMaxForwardMovableLights will increase UBO usage correspondingly and there could be performance penalty if UBO overflows.
// And reducing GMaxForwardMovableLights will save UBO usage for other features using UBO.
// Note: If there are more dynamic lights than GMaxForwardMovableLights, those extra lights will be DISCARDED silently.
// Normally only 4 FForwardMovableLightData members are used on Mobile, which means each light takes 4 x vector4f = 64Bytes UBO and 32 lights takes 2K.
const int32 GMaxForwardMovableLights = 32;
// END META SECTION - LightGrid LightDataUBO

// BEGIN META SECTION - LightGrid LightDataUBO
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT_WITH_CONSTRUCTOR(FForwardMovableLightData, )
	SHADER_PARAMETER_ARRAY(FVector4f, LightPositionAndInvRadius, [GMaxForwardMovableLights])
	SHADER_PARAMETER_ARRAY(FVector4f, LightColorAndIdAndFalloffExponent, [GMaxForwardMovableLights])
	SHADER_PARAMETER_ARRAY(FVector4f, LightDirectionAndShadowMapChannelMask, [GMaxForwardMovableLights])
	SHADER_PARAMETER_ARRAY(FVector4f, SpotAnglesAndSourceRadiusPacked, [GMaxForwardMovableLights])
	SHADER_PARAMETER_ARRAY(FVector4f, LightTangentAndIESDataAndSpecularScale, [GMaxForwardMovableLights])
	SHADER_PARAMETER_ARRAY(FVector4f, RectDataAndVirtualShadowMapIdOrPrevLocalLightIndex, [GMaxForwardMovableLights])
...
public:
	...
	// BEGIN META SECTION - LightGrid LightDataUBO
	FForwardMovableLightData ForwardMovableLightData;
	// END META SECTION - LightGrid LightDataUBO
};
...
public:
	...
	*/
	...
	void UpdateLateLatchData(FRHICommandListImmediate& RHICmdList);
...
public:
	...
	// BEGIN META SECTION - PORT PICO LateLatching
	virtual void ExecuteLateLatching(FRDGBuilder& GraphBuilder) {};
	// END META SECTION - PORT PICO LateLatching
...
public:
	...
	// BEGIN META SECTION - PORT PICO LateLatching
	virtual void ExecuteLateLatching(FRDGBuilder& GraphBuilder) override;
	// END META SECTION - PORT PICO LateLatching
...
protected:
	...
	void PostSceneColorRendering(FRHICommandList& RHICmdList, FViewInfo& View);
	...
	// BEGIN META SECTION - PORT PICO LateLatching
	void ApplyLateLatching(FRHICommandListImmediate& RHICmdList, FViewInfo& View);
	// END META SECTION - PORT PICO LateLatching
	...
	void RenderDeferredSinglePass(FRDGBuilder& GraphBuilder, FSceneTextures& SceneTextures, const FSortedLightSetSceneInfo& SortedLightSet);
	void RenderDeferredMultiPass(FRDGBuilder& GraphBuilder, FSceneTextures& SceneTextures, const FSortedLightSetSceneInfo& SortedLightSet);
	...
	bool HasMotionVectorRenderTargets(const FSceneTextures& SceneTextures);
	...
	// BEGIN META SECTION - Multi-View Per View Viewports / Render Areas
	void SetResolveRects(class FMobileRenderPassParameters* PassParameters, FRenderViewContext& ViewContext);
	// END META SECTION - Multi-View Per View Viewports / Render Areas
private:
	...
	bool bEnableUniformLocalLights;
