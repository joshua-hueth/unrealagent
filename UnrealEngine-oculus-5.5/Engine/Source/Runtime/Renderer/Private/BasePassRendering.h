...
void AddBasePassPixelShader(bool bEnableSkyLight, bool bEnableXRSoftOcclusions, FMaterialShaderTypes& OutShaderTypes, bool bIsForOITPass = false)
{
	...
	if (bIsForOITPass)
	{
		if (bIsForOITPass)
		{
			...
			{
				using FMyShader = TBasePassPS<LightMapPolicyType, true, GBufferLayout, true>;
			...
			{
				using FMyShader = TBasePassPS<LightMapPolicyType, true, GBufferLayout, false>;
				typename FMyShader::FPermutationDomain PermutationVector;
				PermutationVector.template Set<typename FMyShader::FSupportOITDim>(true);
				PermutationId = PermutationVector.ToDimensionValueId();
			}
	...
	if (bEnableXRSoftOcclusions)
	{
		if (bEnableSkyLight)
		{
			OutShaderTypes.AddShaderType<TBasePassPS<LightMapPolicyType, true, GBufferLayout, true>>(PermutationId);
		}
		else
		{
			OutShaderTypes.AddShaderType<TBasePassPS<LightMapPolicyType, false, GBufferLayout, true>>(PermutationId);
		}
	}
	else
	{
		if (bEnableSkyLight)
		{
			OutShaderTypes.AddShaderType<TBasePassPS<LightMapPolicyType, true, GBufferLayout, false>>(PermutationId);
		}
		else
		{
			OutShaderTypes.AddShaderType<TBasePassPS<LightMapPolicyType, false, GBufferLayout, false>>(PermutationId);
		}
	}
}
// END META SECTION - XR Soft Occlusions
...
)
{
	...
	if (PixelShader)
	{
		...
		// BEGIN META SECTION - XR Soft Occlusions
		case GBL_Default:
			AddBasePassPixelShader<LightMapPolicyType, GBL_Default>(bEnableSkyLight, bEnableXRSoftOcclusions, ShaderTypes, bIsForOITPass);
			break;
		case GBL_ForceVelocity:
			AddBasePassPixelShader<LightMapPolicyType, GBL_ForceVelocity>(bEnableSkyLight, bEnableXRSoftOcclusions, ShaderTypes, bIsForOITPass);
			break;
		// END META SECTION - XR Soft Occlusions
		default:
			check(false);
