...
void FDebugViewModeMeshProcessor::UpdateInstructionCount(FDebugViewModeShaderElementData& OutShaderElementData, const FMaterial* InBatchMaterial, FVertexFactoryType* InVertexFactoryType)
{
	...
	if (Scene)
	{
		...
		else // EShadingPath::Mobile
		{
			...
			if (MobileBasePass::GetShaders(LMP_NO_LIGHTMAP, EMobileLocalLightSetting::LOCAL_LIGHTS_DISABLED, 0, false, *InBatchMaterial, InVertexFactoryType, MobileVS, MobilePS))
			// BEGIN END SECTION - XR Soft Occlusions, Bound Uniform Local Lights
			{
