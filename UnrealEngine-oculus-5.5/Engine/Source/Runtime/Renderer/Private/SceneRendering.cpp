...
static TAutoConsoleVariable<int32> CVarEnableLateLatching(
	...
	1,
	TEXT("Whether to allow late-latching functionality."),
	ECVF_RenderThreadSafe | ECVF_Scalability);
// END META SECTION - PORT PICO LateLatching

// BEGIN META SECTION - GPU Occlusion Query Optimizations
// To replace enum { OccludedPrimitiveQueryBatchSize = 16 };
static TAutoConsoleVariable<int32> CVarOccludedPrimitiveQueryBatchSize(
	TEXT("r.OcclusionQueries.OccludedPrimitiveQueryBatchSize"),
	16,
	TEXT("The maximum number of consecutive previously occluded primitives which will be combined into a single occlusion query."),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);
...
// BEGIN META SECTION - GPU Occlusion Query Optimizations
	...
	,	GroupedOcclusionQueries((FSceneViewState*)InitOptions.SceneViewStateInterface, CVarOccludedPrimitiveQueryBatchSize.GetValueOnAnyThread(), bIsInstancedStereoEnabled ? 2 : 1)
// END META SECTION - GPU Occlusion Query Optimizations
...
// BEGIN META SECTION - GPU Occlusion Query Optimizations
	...
	,	GroupedOcclusionQueries((FSceneViewState*)InView->State, CVarOccludedPrimitiveQueryBatchSize.GetValueOnAnyThread(), bIsInstancedStereoEnabled ? 2 : 1)
// END META SECTION - GPU Occlusion Query Optimizations
	,	CustomVisibilityQuery(nullptr)
{
	Init();
...
// BEGIN META SECTION - PORT PICO LateLatching
void FViewInfo::UpdateLateLatchData(FRHICommandListImmediate& RHICmdList)
{
	// Late Latching only support MobileMultiView
	...
	PrevViewInfo.ViewMatrices= ViewMatrices;
	...
	if (!IsInstancedSceneView())
	{
		RHICmdList.UpdateUniformBuffer(ViewUniformBuffer, CachedViewUniformShaderParameters.Get());

		FInstancedViewUniformShaderParameters LocalInstancedViewUniformShaderParameters;
	
		InstancedViewParametersUtils::CopyIntoInstancedViewParameters(LocalInstancedViewUniformShaderParameters, *CachedViewUniformShaderParameters, 0);
		...
		{
			InstancedViewParametersUtils::CopyIntoInstancedViewParameters(LocalInstancedViewUniformShaderParameters, *CachedViewUniformShaderParameters, 1);
		}
		
		RHICmdList.UpdateUniformBuffer(InstancedViewUniformBuffer, &LocalInstancedViewUniformShaderParameters);
	}
}
// END META SECTION - PORT PICO LateLatching
			...
			if (!View.bIsReflectionCapture && !View.bIsSceneCapture )
			{
				...
				if ((GAreScreenMessagesEnabled && !GEngine->bSuppressMapWarnings) && (bIsFrozen || bLocked || bStereoView || bShowAmbientCubemapMipGenSettingsWarning || bAnyWarning))
				{
						...
						(FCanvas& Canvas)
					{
						...
						if (bStereoView)
						{
							// BEGIN META SECTION - Subsampled Layout Fix
							if (bPrimaryStereoView || !bIsMobileMultiViewEnabled)
							{
							...
							// END META SECTION - Subsampled Layout Fix
				...
				// BEGIN META SECTION - Software Occlusion
				// Custom Occlusion debug draw
				if (ViewState && ViewState->CustomOcclusion)
				{
					ViewState->CustomOcclusion->DebugDraw(GraphBuilder, View, Output, CVarCOVisualizeBufferXOffset.GetValueOnAnyThread(), CVarCOVisualizeBufferYOffset.GetValueOnAnyThread());
				}
...
void FSceneRenderer::SetStereoViewport(FRHICommandList& RHICmdList, const FViewInfo& View, float ViewportScale)
{
	...
	{	
		// SecondViewportView is valid when Multview Per View Viewports (MVPVV) are enabled.
		check(ViewportScale == 1.0f)
		...
		GEngine->StereoRenderingDevice->CalculateScissorRect(View.StereoViewIndex, View.ViewRect, ScissorRects[0]);
		GEngine->StereoRenderingDevice->CalculateScissorRect(View.SecondViewportView->StereoViewIndex, View.SecondViewportView->ViewRect, ScissorRects[1]);
		RHICmdList.SetStereoScissor(ScissorRects[0].Min.X, ScissorRects[1].Min.X, ScissorRects[0].Min.Y, ScissorRects[1].Min.Y, ScissorRects[0].Max.X, ScissorRects[1].Max.X, ScissorRects[0].Max.Y, ScissorRects[1].Max.Y);
	}
	else
	{
