...
// BEGIN META SECTION - Decal Rendering VS Compute Reduction
		SHADER_PARAMETER_ARRAY(FMatrix44f, FrustumComponentToClip, [2])
// BEGIN META SECTION - Decal Rendering VS Compute Reduction
...
namespace DecalRendering
{
	...
	void SetShader(FRHICommandList& RHICmdList, FGraphicsPipelineStateInitializer& GraphicsPSOInit, uint32 StencilRef, const FViewInfo& View,
		const FTransientDecalRenderData& DecalData, EDecalRenderStage DecalRenderStage, const FMatrix* FrustumComponentToClip, uint32 FrustumComponentToClipNum, const FScene* Scene)
	// END META SECTION - Decal Rendering VS Compute Reduction
	{
		...
		// Set vertex shader parameters.
		{
			...
			// BEGIN META SECTION - Decal Rendering VS Compute Reduction
			for (uint32_t ViewIndex = 0; ViewIndex < FrustumComponentToClipNum ; ++ViewIndex)
			{
				ShaderParameters.FrustumComponentToClip[ViewIndex] = FMatrix44f(FrustumComponentToClip[ViewIndex]); // LWC_TODO: Precision loss?
			}
			// END META SECTION - Decal Rendering VS Compute Reduction
...
// BEGIN META SECTION - Decal Rendering VS Compute Reduction
	void SetVertexShaderOnly(FRHICommandList& RHICmdList, FGraphicsPipelineStateInitializer& GraphicsPSOInit, const FViewInfo& View, const FMatrix* FrustumComponentToClip, uint32 FrustumComponentToClipNum)
// END META SECTION - Decal Rendering VS Compute Reduction
	{
		TShaderMapRef<FDeferredDecalVS> VertexShader(View.ShaderMap);
