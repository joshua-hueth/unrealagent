	...
	FDeferredDecalPassParameters& PassParameters)
{
	...
	case EDecalRenderTargetMode::DBuffer:
	{
		...
		// BEGIN META SECTION - ViewAspects
		const bool bUseTextureArrays = View.bIsMobileMultiViewEnabled || View.Aspects.IsMobileMultiViewEnabled();
		// END META SECTION - ViewAspects
...
// BEGIN META SECTION - Decal Rendering VS Compute Reduction
static bool RenderPreStencil(FRHICommandList& RHICmdList, const FViewInfo& View, const FMatrix& ComponentToWorldMatrix, const FMatrix* FrustumComponentToClip, uint32 FrustumComponentToClipNum)
// END META SECTION - Decal Rendering VS Compute Reduction
{
...
// BEGIN META SECTION - Decal Rendering VS Compute Reduction
	DecalRendering::SetVertexShaderOnly(RHICmdList, GraphicsPSOInit, View, FrustumComponentToClip, FrustumComponentToClipNum);
// END META SECTION - Decal Rendering VS Compute Reduction
	...
	const auto RenderDecals = [&](uint32 DecalIndexBegin, uint32 DecalIndexEnd, EDecalRenderTargetMode RenderTargetMode)
	{
			...
			[&View, FeatureLevel, ShaderPlatform, DecalIndexBegin, DecalIndexEnd, SortedDecals, DecalRenderStage, RenderTargetMode, bStencilSizeThreshold, bShaderComplexity](FRHICommandList& RHICmdList)
		{
			...
			for (uint32 DecalIndex = DecalIndexBegin; DecalIndex < DecalIndexEnd; ++DecalIndex)
			{
				...
				// BEGIN META SECTION - Decal Rendering VS Compute Reduction
				FMatrix FrustumComponentToClip[2] = { FMatrix::Identity , FMatrix::Identity };
				FrustumComponentToClip[0] = DecalRendering::ComputeComponentToClipMatrix(View, ComponentToWorldMatrix);

				if (View.Aspects.IsMobileMultiViewEnabled())
				{
					const FViewInfo* InstancedView = View.GetInstancedView();
					if (InstancedView != nullptr)
					{
						FrustumComponentToClip[1] = DecalRendering::ComputeComponentToClipMatrix(*InstancedView, ComponentToWorldMatrix);
					}
				}
				// END META SECTION - Decal Rendering VS Compute Reduction
				...
				{
					// BEGIN META SECTION - Decal Rendering VS Compute Reduction
					bThisDecalUsesStencil = RenderPreStencil(RHICmdList, View, ComponentToWorldMatrix, FrustumComponentToClip, UE_ARRAY_COUNT(FrustumComponentToClip));
					// END META SECTION - Decal Rendering VS Compute Reduction
				}
				...
				// BEGIN META SECTION - Decal Rendering VS Compute Reduction
				DecalRendering::SetShader(RHICmdList, GraphicsPSOInit, StencilRef, View, DecalData, DecalRenderStage, FrustumComponentToClip, UE_ARRAY_COUNT(FrustumComponentToClip));
				// END META SECTION - Decal Rendering VS Compute Reduction
