...
static bool FindStereoVelocityTexture(FIntPoint& OutVelocityTextureSize, FTextureRHIRef& OutVelocityTexture, FTextureRHIRef& OutVelocityDepthTexture)
{
	if (IStereoRenderTargetManager* StereoRenderTargetManager = FindStereoRenderTargetManager())
	{
		if (StereoRenderTargetManager->GetRecommendedVelocityTextureSize(OutVelocityTextureSize))
		{
			if (StereoRenderTargetManager->AllocateVelocityTexture(0, OutVelocityTextureSize, PF_FloatRGBA, 1, TexCreate_RenderTargetable | TexCreate_ResolveTargetable | TexCreate_ShaderResource | TexCreate_InputAttachmentRead, OutVelocityTexture))
			{
				StereoRenderTargetManager->AllocateVelocityDepthTexture(0, OutVelocityTextureSize, PF_DepthStencil, 1, TexCreate_DepthStencilTargetable | TexCreate_ShaderResource | TexCreate_InputAttachmentRead, OutVelocityDepthTexture);
				return true;
			}
		}
	}
	
	OutVelocityTexture = nullptr;
	OutVelocityDepthTexture = nullptr;
	return false;
}
// END META SECTION - OpenXR AppSpaceWarp

// BEGIN META SECTION - XR Soft Occlusions
static bool FindEnvironmentDepthTexture_RenderThread(FTextureRHIRef& OutTexture, FTextureRHIRef& OutMinMaxTexture, FVector2f& OutDepthFactors, FMatrix44f OutScreenToDepthMatrices[2], FMatrix44f OutDepthViewProjMatrices[2])
{
	if (IStereoRenderTargetManager* StereoRenderTargetManager = FindStereoRenderTargetManager())
	{
		if (StereoRenderTargetManager->FindEnvironmentDepthTexture_RenderThread(OutTexture, OutMinMaxTexture, OutDepthFactors, OutScreenToDepthMatrices, OutDepthViewProjMatrices))
		{
			return true;
		}
	}
...
public:
	...
	FIntPoint Compute(const FSceneViewFamily& ViewFamily)
	{
		...
		if (bIsVRScene && (FindStereoRenderTargetManager() != nullptr))
		{
			uint32 SizeX = DesiredFamilyExtent.X, SizeY = DesiredFamilyExtent.Y;
			FindStereoRenderTargetManager()->CalculateRenderTargetSize(SizeX, SizeY);
			DesiredFamilyExtent = FIntPoint(SizeX, SizeY);
		}
...
void FMinimalSceneTextures::InitializeViewFamily(FRDGBuilder& GraphBuilder, FViewFamilyInfo& ViewFamily)
{
	...
	else
	{
		...
		if (Desc.NumSamples > 1)
		{
			...
			ETextureCreateFlags ResolveCreateFlags = Config.DepthCreateFlags;
			if (Config.bKeepDepthContent)
			{
				ResolveCreateFlags &= ~TexCreate_Memoryless;
			}
			Desc.Flags = ResolveCreateFlags;
...
void FSceneTextures::InitializeViewFamily(FRDGBuilder& GraphBuilder, FViewFamilyInfo& ViewFamily, FIntPoint FamilySize)
{
	...
	{
		// BEGIN META SECTION - OpenXR AppSpaceWarp
		FTextureRHIRef StereoVelocityRHI, StereoVelocityDepthRHI;
		FIntPoint StereoVelocitySize;
		if (FindStereoVelocityTexture(StereoVelocitySize, StereoVelocityRHI, StereoVelocityDepthRHI))
		{
			SceneTextures.MotionVector = RegisterExternalTexture(GraphBuilder, StereoVelocityRHI, TEXT("ExternalStereoVelocity"));
			if (StereoVelocityDepthRHI)
			{
				SceneTextures.MotionVectorDepth = RegisterExternalTexture(GraphBuilder, StereoVelocityDepthRHI, TEXT("ExternalStereoVelocityDepth"));
			}
		}
		// END META SECTION - OpenXR AppSpaceWarp
	}
	...
	if (FindEnvironmentDepthTexture_RenderThread(EnvironmentDepthRHI, EnvironmentDepthMinMaxRHI, SceneTextures.DepthFactors, SceneTextures.ScreenToDepthMatrices, SceneTextures.DepthViewProjMatrices))
	{
		...
		SceneTextures.EnvironmentDepthMinMaxTexture = RegisterExternalTexture(GraphBuilder, EnvironmentDepthMinMaxRHI, TEXT("EnvironmentDepthMinMax"));
	}
	// END META SECTION - XR Soft Occlusions
	...
	FMobileSceneTextureUniformParameters& SceneTextureParameters)
{
	...
	if (SceneTextures)
	{
		...
		// BEGIN META SECTION - Debug Primitive Multiview Rendering
		...
		else if (SceneTextures->Depth.Target->Desc.ArraySize > 1)
		{
			SceneTextureParameters.SceneDepthTexture = SystemTextures.BlackArray;
		}
		// END META SECTION - Debug Primitive Multiview Rendering
