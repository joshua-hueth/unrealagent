...
IMPLEMENT_STATIC_UNIFORM_BUFFER_SLOT(ForwardMovableLightData);
IMPLEMENT_STATIC_UNIFORM_BUFFER_STRUCT_EX2(FForwardMovableLightData, "ForwardMovableLightData", ForwardMovableLightData, Static, FShaderParametersMetadata::EUsageFlags::NoEmulatedUniformBuffer);
...
FForwardMovableLightData::FForwardMovableLightData()
{
}
...
{
	const FVector3f LightTranslatedWorldPosition(PreViewTranslation + LightParameters.WorldPosition);
...
FLightGrid LightGridInjection(
	...
	// BEGIN META SECTION - LightGrid NOLightGridIfNoLight
	bool bCullLightsToGrid
	// END META SECTION - LightGrid NOLightGridIfNoLight
)
{
	...
	// BEGIN META SECTION - LightGrid Optimization
	const FViewInfo* SecondaryView = nullptr;
	const FViewInfo* PrimaryView = nullptr;
	if (View.bIsMobileMultiViewEnabled || View.bIsInstancedStereoEnabled)
	{
		if (View.StereoPass == EStereoscopicPass::eSSP_PRIMARY)
		{
			SecondaryView = View.GetInstancedView();
		}
		else if (View.StereoPass == EStereoscopicPass::eSSP_SECONDARY)
		{
			PrimaryView = View.GetPrimaryView();
		}
	}
	...
	if (PrimaryView)
	{
		...
		CulledLightDataGridSRV = PrimaryView->ForwardLightingResources.ForwardLightData->CulledLightDataGrid32Bit;
		CulledLightDataGridUAV = PrimaryView->ForwardLightingResources.CulledLightDataGridUAV;
		NumCulledLightsGridSRV = PrimaryView->ForwardLightingResources.ForwardLightData->NumCulledLightsGrid;
		NumCulledLightsGridUAV = PrimaryView->ForwardLightingResources.NumCulledLightsGridUAV;
	}
	else
	{
		// Allocate cells for both primary and secondary views in one buffer
		int32 StereoMultiplier = SecondaryView ? 2 : 1;
	...
	// BEGIN META SECTION - LightGrid NOLightGridIfNoLight
	// Skip LightGrid pass on mobile forward+ if no local lights nor reflection captures at all.
	// DONOT skip this for deferred rendering where it's expecting resources like NumCulledLightsGrid to be written by this pass even if there is no local light.
	const EShaderPlatform ShaderPlatform = View.GetShaderPlatform();
	bool bEnableNoLightGridIfNoLightOpt = bCullLightsToGrid && IsMobilePlatform(ShaderPlatform) && !IsMobileDeferredShadingEnabled(ShaderPlatform);
	bool bSkipLightGrid = bEnableNoLightGridIfNoLightOpt && (NumLocalLights == 0 && NumReflectionCaptures == 0);
	if (!bSkipLightGrid)
	{
		...
		// BEGIN META SECTION - LightGrid Optimization
		PassParameters->ViewCulledDataOffset = PrimaryView ? MaxNumCells * GMaxCulledLightsPerCell : 0;
		PassParameters->ViewGridCellOffset = PrimaryView ? MaxNumCells : 0;
		// END META SECTION - LightGrid Optimization
		...
		// BEGIN META SECTION - LightGrid Optimization
		if (!PrimaryView) // NumCulledLightsGrid is shared, only clear it on the first pass
		{
	...
	{
		AddClearUAVPass(GraphBuilder, NumCulledLightsGridUAV, 0);
		AddClearUAVPass(GraphBuilder, CulledLightDataGridUAV, 0);
	}
	FLightGrid Output;
...
FComputeLightGridOutput FSceneRenderer::ComputeLightGrid(FRDGBuilder& GraphBuilder, bool bCullLightsToGrid, const FSortedLightSetSceneInfo& SortedLightSet)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		if (bCullLightsToGrid)
		{
			...
			for (int32 SortedIndex = SimpleLightsEnd; SortedIndex < SortedLights.Num(); ++SortedIndex)
			{
				...
				if (LightSceneInfo->ShouldRenderLight(View) ||
					(SecondaryView && LightSceneInfo->ShouldRenderLight(*SecondaryView)) ||
					(PrimaryView && LightSceneInfo->ShouldRenderLight(*PrimaryView)))
				// END META SECTION - LightGrid Optimization
				{
		...
		if (PrimaryView)
		{
			...
			check(PrimaryView->ForwardLightingResources.ForwardLightData->NumLocalLights == NumLocalLightsFinal);
			ForwardLightData->ForwardLocalLightBuffer = PrimaryView->ForwardLightingResources.ForwardLightData->ForwardLocalLightBuffer;
		}
		else
		{
		...
		// BEGIN META SECTION - LightGrid Optimization
		ForwardLightData->CulledBufferOffsetISR = MaxNumCells;
		// END META SECTION - LightGrid Optimization
		// BEGIN META SECTION - LightGrid LightDataUBO
		if(MobilePackLightGridLightDataToUBOEnable(View.GetShaderPlatform()))
		{
			int32 LocalLightNum = ForwardLocalLightData.Num();
			
			#if WITH_DEBUG_VIEW_MODES
			static int32 MaxLocalLightNum = 0; //the max local light number so far
			if(LocalLightNum > GMaxForwardMovableLights && LocalLightNum > MaxLocalLightNum)
			{	//Log once whenever there are more local lights than GMaxForwardMovableLights. 
				UE_LOG(LogRenderer, Warning, TEXT("LightDataUBO: %d out of %d local lights, instead of all, are used for lighting. Consider increasing GMaxForwardMovableLights and build the engine again to use all lights."), GMaxForwardMovableLights, LocalLightNum);
			}
			MaxLocalLightNum = FMath::Max<int32>(MaxLocalLightNum, LocalLightNum);
			#endif //WITH_DEBUG_VIEW_MODES
		
			LocalLightNum = FMath::Min<int32>(LocalLightNum, GMaxForwardMovableLights);
			for (int32 LightIndex = 0; LightIndex < LocalLightNum; LightIndex++)
			{
				View.ForwardLightingResources.ForwardMovableLightData.LightPositionAndInvRadius[LightIndex] = ForwardLocalLightData[LightIndex].LightPositionAndInvRadius;
				View.ForwardLightingResources.ForwardMovableLightData.LightColorAndIdAndFalloffExponent[LightIndex] = ForwardLocalLightData[LightIndex].LightColorAndIdAndFalloffExponent; 
				View.ForwardLightingResources.ForwardMovableLightData.LightDirectionAndShadowMapChannelMask[LightIndex] = ForwardLocalLightData[LightIndex].LightDirectionAndShadowMapChannelMask; 
				View.ForwardLightingResources.ForwardMovableLightData.SpotAnglesAndSourceRadiusPacked[LightIndex] = ForwardLocalLightData[LightIndex].SpotAnglesAndSourceRadiusPacked; 
				View.ForwardLightingResources.ForwardMovableLightData.LightTangentAndIESDataAndSpecularScale[LightIndex] = ForwardLocalLightData[LightIndex].LightTangentAndIESDataAndSpecularScale; 
				View.ForwardLightingResources.ForwardMovableLightData.RectDataAndVirtualShadowMapIdOrPrevLocalLightIndex[LightIndex] = ForwardLocalLightData[LightIndex].RectDataAndVirtualShadowMapIdOrPrevLocalLightIndex; 
			}
		}
		// END META SECTION - LightGrid LightDataUBO
