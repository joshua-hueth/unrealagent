...
#define IMPLEMENT_BASEPASS_PIXELSHADER_TYPE(LightMapPolicyType,LightMapPolicyName,bEnableSkyLight,SkyLightName,GBufferLayout,LayoutName,EnableXRSoftOcclusions,XRSoftOcclusionsName) \
	typedef TBasePassPS<LightMapPolicyType, bEnableSkyLight, GBufferLayout, EnableXRSoftOcclusions> TBasePassPS##LightMapPolicyName##SkyLightName##LayoutName##XRSoftOcclusionsName; \
	IMPLEMENT_MATERIAL_SHADER_TYPE(template<>,TBasePassPS##LightMapPolicyName##SkyLightName##LayoutName##XRSoftOcclusionsName,TEXT("/Engine/Private/BasePassPixelShader.usf"),TEXT("MainPS"),SF_Pixel);
...
#define IMPLEMENT_BASEPASS_LIGHTMAPPED_SHADER_TYPE(LightMapPolicyType,LightMapPolicyName) \
	...
	IMPLEMENT_BASEPASS_PIXELSHADER_TYPE(LightMapPolicyType,LightMapPolicyName,true,Skylight,GBL_Default,,true,XRSoftOcclusions) \
	IMPLEMENT_BASEPASS_PIXELSHADER_TYPE(LightMapPolicyType,LightMapPolicyName,false,,GBL_Default,,true,XRSoftOcclusions) \
	IMPLEMENT_BASEPASS_PIXELSHADER_TYPE(LightMapPolicyType,LightMapPolicyName,true,Skylight,GBL_ForceVelocity,ForceVelocity,true,XRSoftOcclusions) \
	IMPLEMENT_BASEPASS_PIXELSHADER_TYPE(LightMapPolicyType,LightMapPolicyName,false,,GBL_ForceVelocity,ForceVelocity,true,XRSoftOcclusions) \
	IMPLEMENT_BASEPASS_PIXELSHADER_TYPE(LightMapPolicyType, LightMapPolicyName, true, Skylight, GBL_Default,,false,) \
	IMPLEMENT_BASEPASS_PIXELSHADER_TYPE(LightMapPolicyType, LightMapPolicyName, false, , GBL_Default,,false,) \
	IMPLEMENT_BASEPASS_PIXELSHADER_TYPE(LightMapPolicyType, LightMapPolicyName, true, Skylight, GBL_ForceVelocity, ForceVelocity,false,) \
	IMPLEMENT_BASEPASS_PIXELSHADER_TYPE(LightMapPolicyType, LightMapPolicyName, false, , GBL_ForceVelocity, ForceVelocity,false,) \
...
F128BitRTBasePassPS::F128BitRTBasePassPS(const ShaderMetaType::CompiledShaderInitializerType& Initializer) :
	// BEGIN META SECTION - XR Soft Occlusions
	TBasePassPS<TUniformLightMapPolicy<LMP_NO_LIGHTMAP>, false, GBL_Default, false>(Initializer)
	// END META SECTION - XR Soft Occlusions
{}
...
// BEGIN META SECTION - XR Soft Occlusions
void AddUniformBasePassPixelShader(bool bEnableSkyLight, bool bUse128bitRT, FMaterialShaderTypes& OutShaderTypes, bool bEnableXRSoftOcclusions, bool bIsForOITPass = false)
// END META SECTION - XR Soft Occlusions
{
	...
	if (bIsForOITPass)
	{
		...
		{
			using FMyShader = TBasePassPS<TUniformLightMapPolicy<Policy>, true, GBufferLayout, true>;
		...
		{
			using FMyShader = TBasePassPS<TUniformLightMapPolicy<Policy>, true, GBufferLayout, false>;
			typename FMyShader::FPermutationDomain PermutationVector;
			PermutationVector.template Set<typename FMyShader::FSupportOITDim>(true);
			PermutationId = PermutationVector.ToDimensionValueId();
		}
	...
	if (bEnableSkyLight)
	{
		...
		if (bEnableXRSoftOcclusions)
		{
			OutShaderTypes.AddShaderType<TBasePassPS<TUniformLightMapPolicy<Policy>, true, GBufferLayout, true>>(PermutationId);
		}
		else
		{
			OutShaderTypes.AddShaderType<TBasePassPS<TUniformLightMapPolicy<Policy>, true, GBufferLayout, false>>(PermutationId);
		}
		// BEGIN META SECTION - XR Soft Occlusions
	}
	else
	{
		...
		else
		{
			...
			if (bEnableXRSoftOcclusions)
			{
				OutShaderTypes.AddShaderType<TBasePassPS<TUniformLightMapPolicy<Policy>, false, GBufferLayout, true>>(PermutationId);
			}
			else
			{
				OutShaderTypes.AddShaderType<TBasePassPS<TUniformLightMapPolicy<Policy>, false, GBufferLayout, false>>(PermutationId);
			}
			// BEGIN META SECTION - XR Soft Occlusions
		}
	}
...
)
{
	...
	if (PixelShader)
	{
		...
		// BEGIN META SECTION - XR Soft Occlusions
		case GBL_Default:
			AddUniformBasePassPixelShader<Policy, GBL_Default>(bEnableSkyLight, bUse128bitRT, ShaderTypes, bEnableXRSoftOcclusions, bIsForOITPass);
			break;
		case GBL_ForceVelocity:
			AddUniformBasePassPixelShader<Policy, GBL_ForceVelocity>(bEnableSkyLight, bUse128bitRT, ShaderTypes, bEnableXRSoftOcclusions, bIsForOITPass);
			break;
		// END META SECTION - XR Soft Occlusions
		default:
			check(false);
	...
	)
{
	...
	case LMP_PRECOMPUTED_IRRADIANCE_VOLUME_INDIRECT_LIGHTING:
		return GetUniformBasePassShaders<LMP_PRECOMPUTED_IRRADIANCE_VOLUME_INDIRECT_LIGHTING>(Material, VertexFactoryType, bEnableXRSoftOcclusions, FeatureLevel, bEnableSkyLight, bUse128bitRT, GBufferLayout, VertexShader, PixelShader, bIsForOITPass);
	case LMP_CACHED_VOLUME_INDIRECT_LIGHTING:
		return GetUniformBasePassShaders<LMP_CACHED_VOLUME_INDIRECT_LIGHTING>(Material, VertexFactoryType, bEnableXRSoftOcclusions, FeatureLevel, bEnableSkyLight, bUse128bitRT, GBufferLayout, VertexShader, PixelShader, bIsForOITPass);
	case LMP_CACHED_POINT_INDIRECT_LIGHTING:
		return GetUniformBasePassShaders<LMP_CACHED_POINT_INDIRECT_LIGHTING>(Material, VertexFactoryType, bEnableXRSoftOcclusions, FeatureLevel, bEnableSkyLight, bUse128bitRT, GBufferLayout, VertexShader, PixelShader, bIsForOITPass);
	case LMP_LQ_LIGHTMAP:
		return GetUniformBasePassShaders<LMP_LQ_LIGHTMAP>(Material, VertexFactoryType, bEnableXRSoftOcclusions, FeatureLevel, bEnableSkyLight, bUse128bitRT, GBufferLayout, VertexShader, PixelShader, bIsForOITPass);
	case LMP_HQ_LIGHTMAP:
		return GetUniformBasePassShaders<LMP_HQ_LIGHTMAP>(Material, VertexFactoryType, bEnableXRSoftOcclusions, FeatureLevel, bEnableSkyLight, bUse128bitRT, GBufferLayout, VertexShader, PixelShader, bIsForOITPass);
	case LMP_DISTANCE_FIELD_SHADOWS_AND_HQ_LIGHTMAP:
		return GetUniformBasePassShaders<LMP_DISTANCE_FIELD_SHADOWS_AND_HQ_LIGHTMAP>(Material, VertexFactoryType, bEnableXRSoftOcclusions, FeatureLevel, bEnableSkyLight, bUse128bitRT, GBufferLayout, VertexShader, PixelShader, bIsForOITPass);
	case LMP_NO_LIGHTMAP:
		return GetUniformBasePassShaders<LMP_NO_LIGHTMAP>(Material, VertexFactoryType, bEnableXRSoftOcclusions, FeatureLevel, bEnableSkyLight, bUse128bitRT, GBufferLayout, VertexShader, PixelShader, bIsForOITPass);
	// END META SECTION - XR Soft Occlusions
	default:
		check(false);
		return false;
	...
	FSharedBasePassUniformParameters& SharedParameters)
{
	...
	const FSceneTextures* SceneTextures = static_cast<const FViewFamilyInfo*>(View.Family)->GetSceneTexturesChecked();
	SetupEnvironmentDepthUniformParameters(View, SystemTextures, SceneTextures, SharedParameters.EnvironmentDepthParameters);
	// END META SECTION - XR Soft Occlusions
}
	...
	const TArrayView<Nanite::FRasterResults>& NaniteRasterResults)
{
	...
	if (ShouldRenderAnisotropyPass(InViews) && Renderer.GetRendererOutput() == ERendererOutput::FinalSceneColor)
	{
