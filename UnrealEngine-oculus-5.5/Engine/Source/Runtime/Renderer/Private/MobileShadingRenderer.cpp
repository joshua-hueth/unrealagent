...
BEGIN_SHADER_PARAMETER_STRUCT(FMobileRenderPassParameters,)
	...
	SHADER_PARAMETER_RDG_UNIFORM_BUFFER(FForwardMovableLightData, ForwardMovableLightData)
	...
	, bSupportsSimpleLights(bDeferredShading || MobileForwardEnableParticleLights(ShaderPlatform))
{
	...
	const bool bLocalLightsBuffer = MobileLocalLightsBufferEnabled(ShaderPlatform);
	bEnableUniformLocalLights = bEnableClusteredLocalLights && !bDeferredShading && !bLocalLightsBuffer && MobileUniformLocalLightsEnable(ShaderPlatform);
...
void FMobileSceneRenderer::SetupMobileBasePassAfterShadowInit(FExclusiveDepthStencil::Type BasePassDepthStencilAccess, TArrayView<FViewCommands> ViewCommandsPerView, FInstanceCullingManager& InstanceCullingManager)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < AllViews.Num(); ++ViewIndex)
	{
		...
		Pass.DispatchPassSetup(
			Scene,
			View,
			FInstanceCullingContext(PassName, ShaderPlatform, &InstanceCullingManager, ViewIds, View.PrevViewInfo.HZB, InstanceCullingMode),
			EMeshPass::BasePass,
	...
	FInitViewTaskDatas& TaskDatas)
{
	...
	const bool bRequiresUpscale = ((int32)RenderTargetSize.X > FamilySize.X || (int32)RenderTargetSize.Y > FamilySize.Y) && !bIsMobileLDR && !ViewFamily.EngineShowFlags.StereoRendering;
	...
	bAdrenoOcclusionMode = (CVarMobileAdrenoOcclusionMode.GetValueOnAnyThread() != 0);
	...
	bKeepDepthContent =
		...
		// BEGIN META SECTION - SceneDepth capture fix
		bSceneDepthCapture ||
		// END META SECTION - SceneDepth capture fix
		Views[0].AntiAliasingMethod == AAM_TemporalAA ||
		bRequireSeparateViewPass ||
		bIsFullDepthPrepassEnabled ||
		bShouldRenderHZB ||
		// BEGIN META SECTION - GPU Occlusion Query Optimizations
		(bAdrenoOcclusionMode && IsVulkanPlatform(ShaderPlatform) && DoOcclusionQueries()) ||
		// END META SECTION - GPU Occlusion Query Optimizations
...
void FMobileSceneRenderer::Render(FRDGBuilder& GraphBuilder)
{
	...
	// BEGIN META SECTION - PORT PICO LateLatching
	GraphBuilder.RDGRequestMobileLateLatching.AddRaw(this,&FMobileSceneRenderer::ExecuteLateLatching);
	// END META SECTION - PORT PICO LateLatching
	...
	if (bRendererOutputFinalSceneColor)
	{
		...
		if (bDeferredShading ||
			// BEGIN META SECTION - Bound Uniform Local Lights
			(bEnableClusteredLocalLights && !bEnableUniformLocalLights) ||
			// END META SECTION - Bound Uniform Local Lights
			bEnableClusteredReflections)
		{
		...
		if (ViewFamily.bResolveScene)
		{
			if (bRenderToSceneColor && !bTonemapSubpassInline)
			{
				// Finish rendering for each view, or the full stereo buffer if enabled
				{
					...
					// BEGIN META SECTION - HDR FDM
					PostProcessingInputs.ShadingRateTexture = SceneTextures.ShadingRateTexture;
					// END META SECTION - HDR FDM
					...
					for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
					{
						if (Views[ViewIndex].ShouldRenderView())
						{
							...
							if (bTonemapSubpass)
							{
								// BEGIN META SECTION - Tonemap Subpass Depth shader resolve
								AddMobileCustomResolvePass(GraphBuilder, Views[ViewIndex], SceneTextures, ViewFamilyTexture, bTonemapSubpass);
								// END META SECTION  - Tonemap Subpass Depth shader resolve
							}
							else
							{ 
...
void FMobileSceneRenderer::RenderForwardMotionVectors(FRDGBuilder& GraphBuilder, FViewInfo& View, const FSceneTextures& SceneTextures)
{
	...
	if (View.ShouldRenderView())
	{
		...
		for (int32 PassIndex = 0; PassIndex < TotalMotionVectorPasses; ++PassIndex)
		{
			...
			FParallelMeshDrawCommandPass& ParallelMeshPass = View.ParallelMeshDrawCommandPasses[GetMeshPassFromVelocityPass(VelocityPass)];
			...
			MotionVectorRenderTargets[0] = FRenderTargetBinding(SceneTextures.MotionVector, LoadAction);
			MotionVectorRenderTargets.DepthStencil = FDepthStencilBinding(SceneTextures.MotionVectorDepth, LoadAction, FExclusiveDepthStencil::DepthWrite_StencilWrite);
			...
			GraphBuilder.AddPass(
				RDG_EVENT_NAME("ForwardMotionVectorRendering(%s)", VelocityPass == EVelocityPass::Opaque ? TEXT("Opaque") : TEXT("Translucent")),
				PassParameters,
...
bool FMobileSceneRenderer::HasMotionVectorRenderTargets(const FSceneTextures& SceneTextures)
{
...
void FMobileSceneRenderer::SetResolveRects(class FMobileRenderPassParameters* PassParameters, FRenderViewContext& ViewContext)
{
	PassParameters->RenderTargets.ResolveRect = FResolveRect(ViewContext.ViewInfo->ViewRect);

	// Use stereo viewports if supported
	if (GSupportsMultiViewPerViewViewports && Views.Num() > 1 && ViewContext.ViewInfo->bIsMobileMultiViewEnabled)
	{
		...
		static const auto VulkanEnableMultiviewPerViewRenderAreasCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.Vulkan.EnableMultiviewPerViewRenderAreas"));
		if (VulkanEnableMultiviewPerViewRenderAreasCVar && VulkanEnableMultiviewPerViewRenderAreasCVar->GetInt())
		{
...
void FMobileSceneRenderer::RenderForward(FRDGBuilder& GraphBuilder, FRDGTextureRef ViewFamilyTexture, FSceneTextures& SceneTextures, FDBufferTextures& DBufferTextures)
{
	...
	// BEGIN META SECTION - HDR FDM
	SceneTextures.ShadingRateTexture = BasePassRenderTargets.ShadingRateTexture;
	// END META SECTION - HDR FDM
	...
	for (FRenderViewContext& ViewContext : RenderViews)
	{
		...
		// BEGIN META SECTION - OpenXR AppSpaceWarp
		if (View.Family && View.Family->bRenderStereoVelocity && HasMotionVectorRenderTargets(SceneTextures) && !View.bIsSceneCapture && !View.bIsReflectionCapture)
		// END META SECTION - OpenXR AppSpaceWarp
		{
		...
		if (View.bIsSceneCapture || View.bIsReflectionCapture || View.bIsPlanarReflection)
		{
			...
			MobileBasePassTextures.EnvironmentDepthMinMaxTexture = SystemTextures.Black;
		...
		else
		{
			...
			MobileBasePassTextures.EnvironmentDepthMinMaxTexture = SceneTextures.EnvironmentDepthMinMaxTexture;
		...
		// BEGIN META SECTION - LightGrid LightDataUBO
		PassParameters->ForwardMovableLightData = CreateForwardMovableLightData(GraphBuilder, View);
		// END META SECTION - LightGrid LightDataUBO
		...
		// BEGIN META SECTION - Multi-View Per View Viewports / Render Areas
		SetResolveRects(PassParameters, ViewContext);
		// END META SECTION - Multi-View Per View Viewports / Render Areas
...
void FMobileSceneRenderer::RenderForwardSinglePass(FRDGBuilder& GraphBuilder, FMobileRenderPassParameters* PassParameters, FRenderViewContext& ViewContext, FSceneTextures& SceneTextures)
{
	...
	// BEGIN META SECTION - PORT PICO LateLatching
	const ERDGPassFlags SCRLLPassFlags = ViewFamily.bLateLatchingEnabled ? ERDGPassFlags::EnableLateLatching:ERDGPassFlags::None;
	// END META SECTION - PORT PICO LateLatching
	...
	GraphBuilder.AddPass(
		...
		// BEGIN META SECTION - PORT PICO LateLatching
		ERDGPassFlags::Raster | ERDGPassFlags::NeverMerge | SCRLLPassFlags,
		// END META SECTION - PORT PICO LateLatching
		[this, PassParameters, ViewContext, bDoOcclusionInMainPass, &SceneTextures, &GraphBuilder](FRHICommandList& RHICmdList)
	{
	// END META SECTION - GPU Occlusion Query Optimizations
		FViewInfo& View = *ViewContext.ViewInfo;
		...
		if (bTonemapSubpassInline)
		{
			RHICmdList.NextSubpass();
			// BEGIN META SECTION - Tonemap Subpass Depth shader resolve
			RenderMobileCustomResolve(RHICmdList, View, NumMSAASamples, SceneTextures, bTonemapSubpassInline);
			// END META SECTION - Tonemap Subpass Depth shader resolve
		}
	});
...
void FMobileSceneRenderer::RenderForwardMultiPass(FRDGBuilder& GraphBuilder, FMobileRenderPassParameters* PassParameters, FRenderViewContext& ViewContext, FSceneTextures& SceneTextures)
{
	...
	*SecondPassParameters = *PassParameters;
	...
	// BEGIN META SECTION - LightGrid LightDataUBO
	SecondPassParameters->ForwardMovableLightData = CreateForwardMovableLightData(GraphBuilder, View);
	// END META SECTION - LightGrid LightDataUBO
...
void FMobileSceneRenderer::RenderDeferredSinglePass(FRDGBuilder& GraphBuilder, FSceneTextures& SceneTextures, const FSortedLightSetSceneInfo& SortedLightSet)
{
	...
	// BEGIN META SECTION - Mobile Deferred Multiview
	const FViewInfo& MainView = Views[0];
	GVRSImageManager.PrepareImageBasedVRS(GraphBuilder, ViewFamily, SceneTextures);
	FRDGTextureRef NewShadingRateTarget = GVRSImageManager.GetVariableRateShadingImage(GraphBuilder, MainView, FVariableRateShadingImageManager::EVRSPassType::BasePass);
	BasePassRenderTargets.ShadingRateTexture = (!MainView.bIsSceneCapture && !MainView.bIsReflectionCapture && (NewShadingRateTarget != nullptr)) ? NewShadingRateTarget : nullptr;
	// END META SECTION - Mobile Deferred Multiview
...
void FMobileSceneRenderer::PostSceneColorRendering(FRHICommandList& RHICmdList, FViewInfo& View)
{
	if (ViewFamily.ViewExtensions.Num() > 1)
	{
...
// BEGIN META SECTION - PORT PICO LateLatching
void FMobileSceneRenderer::ApplyLateLatching(FRHICommandListImmediate& RHICmdList,FViewInfo& View)
{
	...
	for (int ViewIndex = ViewFamily.Views.Num()-1; ViewIndex >-1 ; ViewIndex--)
	{
		Views[ViewIndex].UpdateLateLatchData(RHICmdList);
	}
...
// BEGIN META SECTION - PORT PICO LateLatching
void FMobileSceneRenderer::ExecuteLateLatching(FRDGBuilder& GraphBuilder)
{
	// Late Latching
	if (ViewFamily.bLateLatchingEnabled)
	{
		// LateLatching is only enabled with multiview
		ApplyLateLatching(GraphBuilder.RHICmdList, Views[0]);
	}
}
// END META SECTION - PORT PICO LateLatching
