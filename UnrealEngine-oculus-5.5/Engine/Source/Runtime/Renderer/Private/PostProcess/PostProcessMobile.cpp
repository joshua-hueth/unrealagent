...
void SetViewport(FRHICommandList& RHICmdList, FScreenPassTextureViewport Viewport)
{
	if (!GSupportsMultiViewPerViewViewports)
	{
		RHICmdList.SetViewport(Viewport.Rect.Min.X, Viewport.Rect.Min.Y, 0.0f, Viewport.Rect.Max.X, Viewport.Rect.Max.Y, 1.0f);
	}
	else
	{
		RHICmdList.SetStereoViewport(Viewport.Rect.Min.X, Viewport.Rect.Min.X, Viewport.Rect.Min.Y, Viewport.Rect.Min.Y, 0.0f, Viewport.Rect.Max.X, Viewport.Rect.Max.X, Viewport.Rect.Max.Y, Viewport.Rect.Max.Y, 1.0f);
		RHICmdList.SetStereoScissor(Viewport.Rect.Min.X, Viewport.Rect.Min.X, Viewport.Rect.Min.Y, Viewport.Rect.Min.Y, Viewport.Rect.Max.X, Viewport.Rect.Max.X, Viewport.Rect.Max.Y, Viewport.Rect.Max.Y);
	}
}
...
FMobileBloomSetupOutputs AddMobileBloomSetupPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FEyeAdaptationParameters& EyeAdaptationParameters, const FMobileBloomSetupInputs& Inputs)
{
	...
	PSShaderParameters->RenderTargets.MultiViewCount = (View.bIsMobileMultiViewEnabled) ? 2 : View.Aspects.IsMobileMultiViewEnabled() ? 1 : 0;
	// END META SECTION - ViewAspects
	
	// BEGIN META SECTION - Mobile HDR FDM
	PSShaderParameters->RenderTargets.ShadingRateTexture = Inputs.ShadingRateTexture;
	...
	{
		// BEGIN META SECTION - Multi-View Per View Viewports / Render Areas
		SetViewport(RHICmdList, OutputViewport);
		// END META SECTION - Multi-View Per View Viewports / Render Areas
