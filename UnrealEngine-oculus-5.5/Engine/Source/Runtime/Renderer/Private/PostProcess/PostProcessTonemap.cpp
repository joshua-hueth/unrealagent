...
FScreenPassTexture AddTonemapPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FTonemapInputs& Inputs)
{
	...
	if (!Output.IsValid())
	{
		...
		{
			OutputDesc = FRDGTextureDesc::Create2D(
		...
		{
			OutputDesc = FRDGTextureDesc::Create2DArray(
				SceneColorViewport.Extent,
				Inputs.SceneColor.TextureSRV->Desc.Texture->Desc.Format,
				FClearValueBinding(FLinearColor(0, 0, 0, 0)),
				GFastVRamConfig.Tonemap | TexCreate_ShaderResource | TexCreate_RenderTargetable | (View.bUseComputePasses ? TexCreate_UAV : TexCreate_None),
	...
	else
	{
		...
		// BEGIN META SECTION - Mobile HDR FDM
		PassParameters->RenderTargets.ShadingRateTexture = Inputs.ShadingRateTexture;
		// END META SECTION - Mobile HDR FDM
...
class FMobileCustomResolvePS : public FGlobalShader
{
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		// BEGIN META SECTION - LDR Tonemap Subpass
		OutEnvironment.SetDefine(TEXT("MOBILE_HDR"), IsMobileHDR());
		// END META SECTION - LDR Tonemap Subpass
	}
};

IMPLEMENT_GLOBAL_SHADER(FMobileCustomResolvePS, "/Engine/Private/PostProcessTonemap.usf", "MobileCustomResolve_MainPS", SF_Pixel);

// BEGIN META SECTION - Tonemap Subpass Depth Resolve
void RenderMobileCustomResolve(FRHICommandList & RHICmdList, const FViewInfo & View, const int32 SubpassMSAASamples, FSceneTextures & SceneTextures, bool bTonemapSubpass)
// END META SECTION - Tonemap Subpass Depth Resolve
{
...
// BEGIN META SECTION - Tonemap Subpass Depth Resolve
void AddMobileCustomResolvePass(FRDGBuilder & GraphBuilder, const FViewInfo & View, FSceneTextures & SceneTextures, FRDGTextureRef ViewFamilyTexture, bool bTonemapSubpass)
// END META SECTION - Tonemap Subpass Depth Resolve
{
...
// BEGIN META SECTION - Tonemap Subpass Depth Resolve
		[&View, &SceneTextures, bTonemapSubpass](FRHICommandListImmediate& RHICmdList)
		{
			const uint32 SubpassMSAASamples = 0u; // not using subpass resolve
			RenderMobileCustomResolve(RHICmdList, View, SubpassMSAASamples, SceneTextures, bTonemapSubpass);
		});
// END META SECTION - Tonemap Subpass Depth Resolve
}
