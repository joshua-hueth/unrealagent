...
FRDGTextureRef CreateCompositeDepthTexture(FRDGBuilder& GraphBuilder, FIntPoint Extent, uint32 NumMSAASamples, bool bMultiView)
{
	FRDGTextureDesc DepthDesc;

	if (!bMultiView)
	{
		DepthDesc = FRDGTextureDesc::Create2D(
			Extent,
			PF_DepthStencil,
	...
	{
		DepthDesc = FRDGTextureDesc::Create2DArray(
			Extent,
			PF_DepthStencil,
			FClearValueBinding::DepthFar,
			TexCreate_ShaderResource | TexCreate_DepthStencilTargetable,
