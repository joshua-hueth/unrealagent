...
// BEGIN META SECTION - Emulated Uniform Buffers
	// List of copies from uniform buffers to global data
	struct FEmulatedUBCopy
	{
		uint32 SourceUBIndex;
		uint32 SourceOffsetInFloats;
		uint32 DestOffsetInFloats;
		uint32 SizeInFloats;
	};
	TArray<FEmulatedUBCopy> EmulatedUBCopies;

	// Lookup table for the first copy for each uniform buffer
	struct FEmulatedUBCopyRange
	{
		uint16 Start;
		uint16 Count;
	};
	TArray<FEmulatedUBCopyRange> EmulatedUBCopyRanges;
...
// BEGIN META SECTION - Emulated Uniform Buffers
inline FArchive& operator<<(FArchive& Ar, FVulkanShaderHeader::FEmulatedUBCopy& Info)
{
...
inline FArchive& operator<<(FArchive& Ar, FVulkanShaderHeader::FEmulatedUBCopyRange& Info)
{
	Ar << Info.Start;
	Ar << Info.Count;
	return Ar;
