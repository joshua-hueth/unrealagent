...
FVulkanBackBuffer::FVulkanBackBuffer(FVulkanDevice& Device, FVulkanViewport* InViewport, EPixelFormat Format, uint32 SizeX, uint32 SizeY, ETextureCreateFlags UEFlags)
	: FVulkanTexture(Device, FRHITextureCreateDesc::Create2D(TEXT("FVulkanBackBuffer"), SizeX, SizeY, Format).SetFlags(UEFlags).DetermineInititialState(), VK_NULL_HANDLE, {})
	, Viewport(InViewport)
{
}
...
void FVulkanCommandListContext::RHISetStereoScissor(float LeftMinX, float RightMinX, float LeftMinY, float RightMinY, float LeftMaxX, float RightMaxX, float LeftMaxY, float RightMaxY)
{
	TStaticArray<VkRect2D, 2> Scissors;

	Scissors[0].offset.x = FMath::FloorToInt(LeftMinX);
	Scissors[0].offset.y = FMath::FloorToInt(LeftMinY);
	Scissors[0].extent.width = FMath::CeilToInt(LeftMaxX - LeftMinX);
	Scissors[0].extent.height = FMath::CeilToInt(LeftMaxY - LeftMinY);

	Scissors[1].offset.x = FMath::FloorToInt(RightMinX);
	Scissors[1].offset.y = FMath::FloorToInt(RightMinY);
	Scissors[1].extent.width = FMath::CeilToInt(RightMaxX - RightMinX);
	Scissors[1].extent.height = FMath::CeilToInt(RightMaxY - RightMinY);

	PendingGfxState->SetMultiScissor(Scissors);
}
