...
{
public:
	FVulkanUniformBuffer(const FRHIUniformBufferLayout* InLayout, const void* Contents, EUniformBufferUsage InUsage, EUniformBufferValidation Validation, bool bInIsEmulated);
	...
	static FVulkanUniformBuffer* Create(FVulkanDevice& Device, const FRHIUniformBufferLayout* Layout, const void* Contents, EUniformBufferUsage Usage, EUniformBufferValidation Validation);
public:
...
public:
	...
	virtual ~FVulkanRealUniformBuffer() override;
...
// BEGIN META SECTION - Emulated Uniform Buffers
			EmulatedUBCopies = InCodeHeader.EmulatedUBCopies;
			EmulatedUBCopyRanges = InCodeHeader.EmulatedUBCopyRanges;
// END META SECTION - Emulated Uniform Buffers
		...
		{
// BEGIN META SECTION - Emulated Uniform Buffers
			check(InCodeHeader.EmulatedUBCopies.Num() == 0);
// END META SECTION - Emulated Uniform Buffers
...
// BEGIN META SECTION - Emulated Uniform Buffers
	...
	inline void SetEmulatedUniformBufferIntoPacked(uint32 BindPoint, TArrayView<const uint8> ConstantData, uint32& InOutPackedUniformBufferStagingDirty)
	{
		// Emulated UBs. Assumes UniformBuffersCopyInfo table is sorted by CopyInfo.SourceUBIndex
		if (BindPoint < (uint32)EmulatedUBCopyRanges.Num())
		{
			FVulkanShaderHeader::FEmulatedUBCopyRange Range = EmulatedUBCopyRanges[BindPoint];
			int32 End = (int32)Range.Start + (int32)Range.Count;
			check(End <= EmulatedUBCopies.Num());
			const uint8* RESTRICT SourceData = ConstantData.GetData();
			uint8* RESTRICT StagingData = PackedUniformBuffers.GetData();
			bool bChanged = false;
			for (int32 Index = Range.Start; Index < End; ++Index)
			{
				const FVulkanShaderHeader::FEmulatedUBCopy& CopyInfo = EmulatedUBCopies[Index];
				check(CopyInfo.SourceUBIndex == BindPoint);
				//check(ByteOffset + NumBytes <= (uint32)PackedUniformBuffers.Num());
				uint32* RESTRICT RawDst = (uint32*)(StagingData + CopyInfo.DestOffsetInFloats * 4);
				uint32* RESTRICT RawSrc = (uint32*)(SourceData + CopyInfo.SourceOffsetInFloats * 4);
				uint32* RawSrcEnd = RawSrc + CopyInfo.SizeInFloats;
				do
				{
					bChanged |= CopyAndReturnNotEqual(*RawDst++, *RawSrc++);
				}
...
// BEGIN META SECTION - Emulated Uniform Buffers
	TArray<FVulkanShaderHeader::FEmulatedUBCopy> EmulatedUBCopies;
	TArray<FVulkanShaderHeader::FEmulatedUBCopyRange> EmulatedUBCopyRanges;
// END META SECTION - Emulated Uniform Buffers
};
