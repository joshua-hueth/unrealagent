...
public:
	...
	void BuildCreateInfo(const FVulkanRenderTargetLayout& RTLayout)
	{
		...
		if (bCustomResolveSubpass)
		{
			...
			// BEGIN META SECTION - Enable Shading Rate Attachment in Tonemap Subpass
			if (RTLayout.GetHasResolveAttachments())
			{
				ColorAttachments3[0].attachment = ResolveAttachmentReferences.Num() > 1 ? ResolveAttachmentReferences[1].attachment : ResolveAttachmentReferences[0].attachment;
			}
			else
			{
			...
			{
				InputAttachments3[0].attachment = DepthInputAttachment;
				InputAttachments3[0].layout = DepthInputAttachmentLayout;
				InputAttachments3[0].SetAspect(DepthInputAspectMask);
			}
			// END META SECTION - Tonemap Subpass Depth Resolve
			...
			if (Device.GetOptionalExtensions().HasQcomRenderPassShaderResolve)
			{
				SubpassDesc.flags |= VK_SUBPASS_DESCRIPTION_SHADER_RESOLVE_BIT_QCOM;
			}


			if (bResolveDepth && RTLayout.GetHasDepthStencilResolve())
			{
				DepthStencilAttachment2.attachment = RTLayout.GetDepthStencilResolveAttachmentReference()->attachment;
				DepthStencilAttachment2.layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
				DepthStencilAttachment2.SetAspect(VK_IMAGE_ASPECT_DEPTH_BIT);

				SubpassDesc.SetDepthStencilAttachment(&DepthStencilAttachment2);
			}
#endif
			// END META SECTION - Tonemap Subpass Depth Resolve
...
private:
	...
	// BEGIN META SECTION - Tonemap Subpass Depth Resolve
	// shader resolve for depth subpass tonemap
	TAttachmentReferenceClass DepthStencilAttachment2;
	// END META  SECTION - Tonemap Subpass Depth Resolve
