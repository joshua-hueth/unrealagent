...
public:
	...
	FVulkanQCOMFragmentDensityMapOffsetExtension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_QCOM_FRAGMENT_DENSITY_MAP_OFFSET_EXTENSION_NAME, VULKAN_EXTENSION_ENABLED)
	{
	}
...
class FVulkanQCOMMultiviewPerViewViewportsExtension : public FVulkanDeviceExtension
{
public:

	FVulkanQCOMMultiviewPerViewViewportsExtension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_QCOM_MULTIVIEW_PER_VIEW_VIEWPORTS_EXTENSION_NAME, VULKAN_SUPPORTS_MULTIVIEW_PER_VIEW_VIEWPORTS)
	{
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		ZeroVulkanStruct(MultiviewPerViewViewportsFeatures, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MULTIVIEW_PER_VIEW_VIEWPORTS_FEATURES_QCOM);
		AddToPNext(PhysicalDeviceFeatures2, MultiviewPerViewViewportsFeatures);
	}

	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		ExtensionFlags.HasQcomMultiviewPerViewViewports = MultiviewPerViewViewportsFeatures.multiviewPerViewViewports;
	}

	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo) override final
	{
		if (MultiviewPerViewViewportsFeatures.multiviewPerViewViewports == VK_TRUE)
		{
			AddToPNext(DeviceCreateInfo, MultiviewPerViewViewportsFeatures);
		}
	}
...
class FVulkanQCOMMultiviewPerViewRenderAreasExtension : public FVulkanDeviceExtension
{
public:

	FVulkanQCOMMultiviewPerViewRenderAreasExtension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_QCOM_MULTIVIEW_PER_VIEW_RENDER_AREAS_EXTENSION_NAME, VULKAN_SUPPORTS_MULTIVIEW_PER_VIEW_RENDER_AREAS)
	{
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		ZeroVulkanStruct(MultiviewPerViewRenderAreasFeatures, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MULTIVIEW_PER_VIEW_RENDER_AREAS_FEATURES_QCOM);
		AddToPNext(PhysicalDeviceFeatures2, MultiviewPerViewRenderAreasFeatures);
	}

	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		ExtensionFlags.HasQcomMultiviewPerViewRenderAreas = MultiviewPerViewRenderAreasFeatures.multiviewPerViewRenderAreas;
	}

	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo) override final
	{
		if (MultiviewPerViewRenderAreasFeatures.multiviewPerViewRenderAreas == VK_TRUE)
		{
			AddToPNext(DeviceCreateInfo, MultiviewPerViewRenderAreasFeatures);
		}
	}

private:
	VkPhysicalDeviceMultiviewPerViewRenderAreasFeaturesQCOM MultiviewPerViewRenderAreasFeatures;
};
...
FVulkanDeviceExtensionArray FVulkanDeviceExtension::GetUESupportedDeviceExtensions(FVulkanDevice* InDevice, uint32 ApiVersion)
{
	...
	// BEGIN META SECTION - Multi-View Per View Viewports / Render Areas
	ADD_CUSTOM_EXTENSION(FVulkanQCOMMultiviewPerViewViewportsExtension);
	ADD_CUSTOM_EXTENSION(FVulkanQCOMMultiviewPerViewRenderAreasExtension);
	// END META SECTION - Multi-View Per View Viewports / Render Areas
