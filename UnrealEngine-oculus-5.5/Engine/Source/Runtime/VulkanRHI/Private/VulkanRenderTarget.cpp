...
{
	// this initlize differently from 
	...
	, MultiViewCount(RPInfo.MultiViewCount)
{
	...
	for (int32 Index = 0; Index < NumColorRenderTargets; ++Index)
	{
		...
		// this causes black buffer artifact if the shader resolve targt load op is clear
		CurrDesc.loadOp = bCustomResolveAttachment ? VK_ATTACHMENT_LOAD_OP_DONT_CARE : RenderTargetLoadActionToVulkan(GetLoadAction(ColorEntry.Action));
	...
	, MultiViewCount(0)
{
	...
	// BEGIN META SECTION - Tonemap Subpass Depth shader resolve
	bool bCustomResolvePass = Initializer.SubpassHint == ESubpassHint::CustomResolveSubpass;
	// END META SECTION - Tonemap Subpass Depth shader resolve
	for (uint32 Index = 0; Index < Initializer.RenderTargetsEnabled; ++Index)
	{
		...
		if (UEFormat != PF_Unknown)
		{
			...
			// BEGIN META SECTION - Tonemap Subpass Depth shader resolve
			CurrDesc.storeOp = bCustomResolveAttachment ? VK_ATTACHMENT_STORE_OP_STORE : VK_ATTACHMENT_STORE_OP_DONT_CARE;
			// END META SECTION - Tonemap Subpass Depth shader resolve
			CurrDesc.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
			CurrDesc.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
			...
			ColorReferences[NumColorAttachments].layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
			// BEGIN META SECTION - Tonemap Subpass Depth shader resolve
			if (CurrDesc.samples > VK_SAMPLE_COUNT_1_BIT && !bCustomResolvePass)
			// END META SECTION - Tonemap Subpass Depth shader resolve
			{
				Desc[NumAttachmentDescriptions + 1] = Desc[NumAttachmentDescriptions];
				Desc[NumAttachmentDescriptions + 1].samples = VK_SAMPLE_COUNT_1_BIT;
