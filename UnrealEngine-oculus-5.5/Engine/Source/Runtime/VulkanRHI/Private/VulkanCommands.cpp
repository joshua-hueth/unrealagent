...
void FVulkanCommandListContext::RHISetUniformBufferDynamicOffset(FUniformBufferStaticSlot InSlot, uint32 InOffset)
{
	...
	FVulkanUniformBuffer* RawUniformBuffer = ResourceCast(GlobalUniformBuffers[InSlot]);
	checkf(!GVulkanUseEmulatedUBs, TEXT("GPUScene and Emulated UBs are incompatible"));
	check(!RawUniformBuffer || !RawUniformBuffer->bIsEmulated);
	FVulkanRealUniformBuffer* UniformBuffer = static_cast<FVulkanRealUniformBuffer*>(RawUniformBuffer);
...
// BEGIN META SECTION - Emulated Uniform Buffers
	if (!UniformBuffer->bIsEmulated && static_cast<FVulkanRealUniformBuffer*>(UniformBuffer)->IsUniformView())
	{
		static_cast<FVulkanRealUniformBuffer*>(UniformBuffer)->SetupUniformBufferView();
	}
// END META SECTION - Emulated Uniform Buffers
...
// BEGIN META SECTION - Emulated Uniform Buffers
		if (!UniformBuffer->bIsEmulated)
		{
			...
			{
				PendingGfxState->SetUniformBuffer<true>(Stage, BufferIndex, RealUniformBuffer);
			}
			else
			{
				check(DescriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
				PendingGfxState->SetUniformBuffer<false>(Stage, BufferIndex, RealUniformBuffer);
			}
		}
		else
		{
...
// BEGIN META SECTION - Emulated Uniform Buffers
		if (!UniformBuffer->bIsEmulated)
		{
			...
			if (DescriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC)
			{
				State.SetUniformBuffer<true>(ShaderStage::Compute, BufferIndex, RealUniformBuffer);
			}
			else
			{
				check(DescriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
				State.SetUniformBuffer<false>(ShaderStage::Compute, BufferIndex, RealUniformBuffer);
			}
		}
		else
		{
