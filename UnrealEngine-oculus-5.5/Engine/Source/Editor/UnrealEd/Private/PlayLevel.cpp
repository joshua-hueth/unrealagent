...
void UEditorEngine::CreateNewPlayInEditorInstance(FRequestPlaySessionParams &InRequestParams, const bool bInDedicatedInstance, const EPlayNetMode InNetMode)
{
	...
	else
	{
		...
		InRequestParams.EditorPlaySettings->GetPlayNumberOfClients(PlayNumberOfClients);
		if (PlayNumberOfClients > 1)
		{
			TArray<FString>& MPAdditionalLaunchParameters = InRequestParams.EditorPlaySettings->GetMPAdditionalLaunchParameters();
			int32 NumClientInstancesCreated = PlayInEditorSessionInfo->NumClientInstancesCreated;
			if (MPAdditionalLaunchParameters.Num() > NumClientInstancesCreated)
			{
				if (NumClientInstancesCreated > 0) 
				{
					// replace AdditionalLaunchParameters of previous player with that of current player
					FString CommandLine = FCommandLine::Get();
					CommandLine = CommandLine.Replace(*MPAdditionalLaunchParameters[NumClientInstancesCreated - 1], *MPAdditionalLaunchParameters[NumClientInstancesCreated]);
					FCommandLine::Set(*CommandLine);
				}
				else
				{
					FCommandLine::Append(*MPAdditionalLaunchParameters[NumClientInstancesCreated]);
				}
			}
		}
